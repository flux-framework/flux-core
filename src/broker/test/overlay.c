                            ...
                            flux_watcher_f cb)
{
    ...
    if (!(ctx->ov = overlay_create (h, ctx->name, ctx->attrs, zctx, ctx->uri)))
...
void wrongness (flux_t *h)
{
    ...
    if (!(attrs = attr_create ()))
        BAIL_OUT ("attr_create failed");
    errno = 0;
    ok (overlay_create (NULL, "test0", attrs, zctx, "interthread://x") == NULL
        && errno == EINVAL,
        "overlay_create h=NULL fails with EINVAL");
    errno = 0;
    ok (overlay_create (h, "test0", NULL, zctx, "interthread://x") == NULL
        && errno == EINVAL,
    ...
    if (!(attrs = attr_create ()))
        BAIL_OUT ("attr_create failed");
    if (!(ov = overlay_create (h, "test0", attrs, zctx, "interthread://x")))
