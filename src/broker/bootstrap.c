 ...
 */
static int bootstrap_setattrs (struct bootstrap *boot, flux_error_t *errp)
{
    attr_t *attrs = boot->ctx->attrs;
    bool under_flux = true;
    ...
    if (under_flux) {
        ...
        if (!val)
            under_flux = false; // save future flux.* lookups that will fail
     ...
     */
    if (under_flux && !getattr (attrs, "tbon.interface-hint")) {
