...
class Calendar(object):
    ...
    def parseDate(self, dateString, sourceTime=None):
        ...
        for i in range(0, 3):
            ...
            if n >= 0:
                d[c] = n
                accuracy.append(
                    {"m": pdtContext.ACU_MONTH, "d": pdtContext.ACU_DAY, "y": pdtContext.ACU_YEAR}[
                        c
                    ]
                )
    ...
    def parseDateText(self, dateString, sourceTime=None):
        ...
        debug and log.debug(
            "parseDateText returned " "mth %d dy %d yr %d sourceTime %s", mth, dy, yr, sourceTime
        )
    ...
    def _evalModifier(self, modifier, chunk1, chunk2, sourceTime):
        ...
        debug and log.debug(
            "modifier [%s] chunk1 [%s] " "chunk2 [%s] unit [%s]", modifier, chunk1, chunk2, unit
        )
        ...
        if unit in self.ptc.units["months"]:
            ...
            if modifier == "eod":
                ...
                diff = self._CalculateDOWDelta(
                    wd, wkdy, dowOffset, self.ptc.DOWParseStyle, self.ptc.CurrentDOWParseStyle
                )
                start = datetime.datetime(yr, mth, dy, startHour, startMinute, startSecond)
            ...
            debug and log.debug("check for modifications " "to source time [%s] [%s]", chunk1, unit)
    ...
    def _evalDT(self, datetimeString, sourceTime):
        ...
        if sourceTime is None:
            ...
            if sourceTime is not None:
                ctx.updateAccuracy(
                    ctx.ACU_YEAR, ctx.ACU_MONTH, ctx.ACU_DAY, ctx.ACU_HOUR, ctx.ACU_MIN, ctx.ACU_SEC
                )
    ...
    def _partialParseModifier(self, s, sourceTime):
        ...
        if parseStr:
            debug and log.debug("found (modifier) [%s][%s][%s]", parseStr, chunk1, chunk2)
    ...
    def _partialParseMeridian(self, s, sourceTime):
        ...
        if parseStr:
            debug and log.debug("found (meridian) [%s][%s][%s]", parseStr, chunk1, chunk2)
    ...
    def parseDT(self, datetimeString, sourceTime=None, tzinfo=None, version=None):
        ...
        # Punt
        time_struct, ret_code = self.parse(datetimeString, sourceTime=sourceTime, version=version)
    ...
    def parse(self, datetimeString, sourceTime=None, version=None):
        ...
        with self.context() as ctx:
            ...
            while s:
                ...
                debug and log.debug("hasDate: [%s], hasTime: [%s]", ctx.hasDate, ctx.hasTime)
...
def _initSymbols(ptc):
    ...
    for idx, xm in enumerate(ptc.locale.meridian[:2]):
        ...
        if xm:
            lxm = xm.lower()
            target.extend((xm[0], "{0}.{1}.".format(*xm), lxm, lxm[0], "{0}.{1}.".format(*lxm)))
