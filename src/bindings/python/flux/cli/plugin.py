...
class CLIPluginRegistry:
    ...
    def _add_plugins(self, path, program):
        ...
        for entry in entries:
            ...
            if (
                ...
                ## Using a setter instead of another argument
                ##  prevents us from breaking currently used plugins
    ...
    def print_plugins(self):
        ...
        for plugin in self.plugins:
            print(f"{plugin}: ")
            print(f"Loaded from: {plugin._load_path}")
            for option in plugin.options:
    ...
    def _load_plugins(self, program):
        ...
        for plugin in self.plugins[:]:
            for option in plugin.options:
                # n.b. this is based on a destination conflict, not a name
                #  conflict (i.e. the prefix may not be considered if the
                #  default is used)
                try:
                    if option_dests[option.kwargs["dest"]]:
                        self.plugins.remove(plugin)
                    break
                except KeyError:
                    option_dests[option.kwargs["dest"]] = option
        self.options = option_dests.values()
        return self
