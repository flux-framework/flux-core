...
class AllocRequest:
    ...
    def success(self, R, annotations=None, clear=True):
        ...
        Args:
            R (dict): The allocated resources as a parsed R JSON object.
...
class Scheduler(BrokerModule):
    ...
    - ``resource_class`` — resource pool class inheriting
      :class:`~flux.resource.pool.ResourcePool` (default
      :class:`~flux.resource.Rv1Pool`); override or pass
      ``resource-class=Rv1RlistPool`` at load time to use the C-backed
      implementation
    - ``FORECAST_PERIOD`` — minimum seconds between :meth:`forecast` calls
      (default 1.0); end users may override at load time with
      ``forecast-period=N``.

    The ``alloc-mode=`` argument (e.g. ``alloc-mode=best-fit``) is parsed
    automatically and stored as :attr:`_alloc_mode` for subclasses to pass to
    :meth:`~flux.resource.pool.ResourcePool.alloc`.  Supported modes are taken
    from the pool class's ``supported_alloc_modes`` attribute; an unsupported
    mode raises :exc:`ValueError` at startup so module load fails immediately.
    :class:`~flux.resource.Rv1Pool` only supports ``worst-fit``;
    :class:`~flux.resource.Rv1RlistPool` also supports ``best-fit`` and
    ``first-fit``.
    ...
    The current resource pool is available as :attr:`resources`, an instance
    of :attr:`resource_class`.  Resource state is updated automatically from
    ``resource.acquire`` and :meth:`resource_update` is called after each update.
    """
    ...
    #: EWMA smoothing factor for the adaptive scheduling timer.  Higher values
    #: react faster to changes in burst rate and scheduling cost; lower values
    #: are more stable.  0.25 converges in roughly 4 samples.
    ...
    #: Resource pool implementation class; must inherit
    #: :class:`~flux.resource.pool.ResourcePool`.
    #: Defaults to :class:`~flux.resource.Rv1Pool`, the pure-Python
    #: implementation which supports GPU scheduling.  Override on the subclass
    #: or pass ``resource-class=Rv1RlistPool`` at load time to use the
    ...
    def __init__(self, h, *args):
        ...
        # _sched_duration_ewma: EWMA of schedule() execution time.
        # _sched_interval_ewma: EWMA of time between _request_schedule() calls.
        ...
        self._alloc_mode = None  # None → pool default (worst-fit)
        self._pending_args = []
        for arg in args:
            if arg.startswith("queue-depth="):
                ...
                if val == "unlimited":
                    ...
                    try:
                        ...
                        raise ValueError(
                            f"queue-depth must be a positive integer or "
                            f"'unlimited', got {val!r}"
                        )
            ...
            elif arg.startswith("resource-class="):
                name = arg[15:]
                if name == "Rv1RlistPool":
                    self.resource_class = Rv1RlistPool
                elif name == "Rv1Pool":
                    self.resource_class = Rv1Pool
                else:
                    raise ValueError(f"unknown resource-class: {name!r}")
            elif arg.startswith("alloc-mode="):
                self._alloc_mode = arg[11:]
        ...
        if self._alloc_mode is not None:
            supported = getattr(self.resource_class, "supported_alloc_modes", None)
            if supported is not None and self._alloc_mode not in supported:
                raise ValueError(
                    f"alloc-mode={self._alloc_mode!r} not supported by "
    ...
    def _reject_unknown_args(self):
        ...
        if self._pending_args:
            raise ValueError(f"unknown argument: {self._pending_args[0]!r}")
    ...
    @property
    def resources(self):
        """The current resource pool (an instance of :attr:`resource_class`)."""
    ...
    def _request_schedule(self):
        ...
        EWMA.  Arms the one-shot scheduling timer if it is not already pending;
        subsequent calls while the timer is armed are recorded for EWMA
        purposes but do not re-arm the timer, coalescing all requests in the
        window into a single :meth:`schedule` call.
    ...
    def _on_sched_timer(self, *_):
        ...
        if self._sched_delay != old_delay:
            if self._sched_delay > 0:
                ...
                self.handle.log(syslog.LOG_DEBUG, "sched: burst ended, delay=0")
    ...
    def _alloc_commit_continuation(self, future, args):
        ...
        try:
            ...
            self.handle.log(syslog.LOG_ERR, f"alloc: KVS commit failed: {exc}")
            self.stop_error()
            return
    ...
    def hello(self, jobid, priority, userid, t_submit, R):
        ...
        Args:
            ...
            R: The job's current allocation (instance of :attr:`resource_class`).
        """
    ...
    def alloc(self, request, jobid, priority, userid, t_submit, jobspec):
        ...
        Parses *jobspec*, checks GPU support, and pushes a
        :class:`PendingJob` onto :attr:`_queue`.  Override to change how
        jobs are parsed or queued.
    ...
    def free(self, jobid, R, final=False):
        ...
        as removing per-job state or logging.

        Args:
            jobid (int): The job ID.
            R: The released resources (instance of :attr:`resource_class`).
            final (bool): True if this is the final free for the job.
        """
        self.resources.free(jobid)
    ...
    def feasibility_check(self, msg, jobspec):
        ...
        try:
            ...
            lib.flux_respond_error(
                ...
                msg.handle,
                exc.errno,
                exc.strerror.encode() if exc.strerror else b"",
            )
            return
        self.handle.respond(msg, None)
    ...
    def run(self):
        ...
        try:
            ...
            self.handle.log(syslog.LOG_ERR, f"initialization failed: {exc}")
            return
        super().run()
    ...
    def _handle_alloc(self, msg):
        try:
            ...
            self.handle.log(syslog.LOG_ERR, f"alloc callback raised: {exc}")
            self.stop_error()
    ...
    def _handle_free(self, msg):
        try:
            p = msg.payload
            R = self.resource_class(p["R"])
            self.free(p["id"], R, final=p.get("final", False))
            self._request_schedule()
        except Exception as exc:
            self.handle.log(syslog.LOG_ERR, f"free callback raised: {exc}")
            self.stop_error()
    ...
    def _handle_cancel(self, msg):
        try:
            ...
            self.handle.log(syslog.LOG_ERR, f"cancel callback raised: {exc}")
            self.stop_error()
    ...
    def _handle_prioritize(self, msg):
        try:
            ...
            self.handle.log(syslog.LOG_ERR, f"prioritize callback raised: {exc}")
            self.stop_error()
    ...
    def _handle_resource_status(self, msg):
        try:
            ...
            all_pool = pool.copy()
            all_pool.mark_up("all")
            resp = {
                "all": all_pool.to_dict(),
                "allocated": pool.copy_allocated().to_dict(),
                "down": pool.copy_down().to_dict(),
            }
            self.handle.respond(msg, resp)
        except Exception as exc:
            self.handle.log(syslog.LOG_ERR, f"resource-status failed: {exc}")
            lib.flux_respond_error(
    ...
    def _handle_expiration(self, msg):
        try:
            ...
            self.handle.log(syslog.LOG_ERR, f"expiration callback raised: {exc}")
            lib.flux_respond_error(
                self.handle.handle, msg.handle, errno.EINVAL, str(exc).encode()
            )
    ...
    def _handle_feasibility(self, msg):
        try:
            ...
            self.handle.log(syslog.LOG_ERR, f"feasibility callback raised: {exc}")
    ...
    def _acquire_resources(self):
        ...
        self._resources = self.resource_class(R)
    ...
    def _acquire_continuation(self, future):
        ...
        try:
            ...
            self.handle.log(
                syslog.LOG_ERR, f"exiting due to resource update failure: {exc}"
            )
            self.stop()
            return
        self._apply_resource_update(data)
    ...
    def _sched_hello(self):
        ...
        while True:
            ...
            try:
                ...
                self.handle.log(
                    syslog.LOG_ERR, f"hello: failed to look up R for job {jobid}"
                )
                f.reset()
                continue
            ...
            if free_ranks:
                ...
                rset = self.resource_class(R_dict)
                rset.remove_ranks(IDset(free_ranks))
                R_dict = rset.to_dict()

            try:
                R = self.resource_class(R_dict)
                self.hello(
                    jobid,
                    data["priority"],
                    data["userid"],
                ...
                self.handle.log(
                    syslog.LOG_ERR,
                    f"hello callback failed for job {jobid}: {exc}",
                )
                self.handle.log(
                    syslog.LOG_INFO,
    ...
    def _sched_ready(self):
        ...
        count = resp.get("count", 0) if resp else 0
        self.handle.log(
            syslog.LOG_DEBUG,
            f"ready: queue_depth={depth}, count={count},"
            f" resource_class={self.resource_class.__name__},"
            f" resources={self.resources.copy().dumps()}",
        )
