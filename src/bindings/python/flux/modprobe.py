...
def default_flux_confdir():
    """Derive sysconfdir as parent of builtin rc1_path"""
...
class Module(Task):
    ...
    def __init__(self, conf):
        """Initialize a module task from modules.toml entry
        ...
        self.name = conf["name"]
        self.module = conf.get("module", self.name)
    ...
    def _load(self, context):
        ...
        payload = {"path": self.module, "args": self.args}
...
class Context:
    ...
    def __init__(self, modprobe, verbose=False):
...
class Modprobe:
    def __init__(self, confdir=None, timing=False, verbose=False):
        self.exitcode = 0
        self.timing = None
        self.t0 = None
        self._locals = None
        ...
        self.taskdb = TaskDB()
        self.context = Context(self, verbose=verbose)
        self.handle = self.context.handle
        self.rank = self.handle.get_rank()

        self.confdir = confdir
        if self.confdir is None:
            self.confdir = default_flux_confdir()
    ...
    def _modprobe_path_expand(self, path=None, name="modules", ext="toml"):
        if path is None:
        ...
        for directory in dirs:
            if Path(directory).exists():
                files.extend(glob.glob(f"{directory}/{name}.d/*.{ext}"))
        return files
    ...
    def configure_modules(self, path=None):
        ...
        Loads all modules TOML files in the following order:
            1. From ``path`` if provided, default path={sysconfdir}/modules.toml
            2. From dirname(``path``)/modules.d/*.toml
            3. for dir in FLUX_MODPROBE_PATH: {dir}/modules.d/*.toml
        """
