...
class _TreePoolV1(Rv1Pool):
    ...
    def __init__(self, *args, **kwargs):
        ...
        remapped = {rank_remap[o]: (lvls, topo) for o, (lvls, topo) in raw_groups.items()}
    ...
    def _container_exclusive_select(self, candidates, request):
        ...
        for rank, info, free_cores, free_gpus in candidates:
            ...
            free_groups = [
                (gc, gg)
                for gc, gg in groups
                if gc <= free_cores and gg <= free_gpus
            ]
            if not free_groups:
                continue
            max_take = (
...
class TreePool(ResourcePool):
    ...
    def __init__(self, R, log=None, **kwargs):
        ...
        if isinstance(R, ResourcePoolImplementation):
            super().__init__(R)
            return
        if isinstance(R, str):
            R = json.loads(R)
...
def amend(R, hwloc_xml=None):
    ...
    result = subprocess.run(
        cmd, input=hwloc_xml, capture_output=True, text=True, check=True
    )
