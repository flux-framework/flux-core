...
class QueueResourceCounts:
    ...
    Attributes:
        ...
        ncores (int): Count of cores
        ngpus (int): Count of gpus
    """
...
class QueueInfo:
    ...
    def __init__(self, name, config, resources, enabled, started, default):
        ...
        self.defaults = QueueDefaults(
            duration=parse_fsd(self.policy_default("duration"))
        )
        self.limits = QueueLimits(
            min=QueueResourceCounts(
                nnodes=self.size_limit("nnodes", False),
                ncores=self.size_limit("ncores", False),
                ngpus=self.size_limit("ngpus", False),
            ),
            max=QueueResourceCounts(
                nnodes=self.size_limit("nnodes"),
                ncores=self.size_limit("ncores"),
                ngpus=self.size_limit("ngpus"),
            ),
            duration=parse_fsd(self.policy_system_limit("duration")),
        )

    def size_limit(self, key, maximum=True):
    ...
    def policy_default(self, key, default="inf"):
        try:
...
class QueueList:
    ...
    Args:
        ...
        queues (list): Optional list of queue names to target
    """
