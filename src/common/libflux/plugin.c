...
flux_plugin_handler_create (const char *topic, flux_plugin_f cb)
{
                             ...
                             flux_plugin_f cb)
{
    ...
    if (!(h = flux_plugin_handler_create (topic, cb)))
        return plugin_seterror (p, errno, NULL);
                          ...
                          const struct flux_plugin_handler t[])
{
    ...
    if (name && flux_plugin_set_name (p, name))
        return -1;
    while (t->topic) {
        if (flux_plugin_add_handler (p, t->topic, t->cb) < 0)
            return -1;
        t++;
    }
    return 0;
