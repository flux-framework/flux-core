...
void check_cornercase (void)
{
    ...
    errno = 0;
    ok (flux_msg_route_pop (NULL) == NULL && errno == EINVAL,
        "flux_msg_route_pop returns NULL errno EINVAL on msg = NULL");
    errno = 0;
    ok (flux_msg_route_pop (msg) == NULL && errno == EPROTO,
        "flux_msg_route_pop returns NULL errno EPROTO on msg "
        "w/o routes enabled");
    errno = 0;
    ok (flux_msg_route_delete (NULL) == -1 && errno == EINVAL,
        "flux_msg_route_delete returns -1 errno EINVAL on id = NULL");
    errno = 0;
    ok (flux_msg_route_delete (msg) == -1 && errno == EPROTO,
        "flux_msg_route_delete returns -1 errno EPROTO on msg "
        "w/o routes enabled");
    ok (flux_msg_route_first (NULL) == NULL && errno == EINVAL,
        "flux_msg_route_first returns NULL errno EINVAL on msg = NULL");
    errno = 0;
    ok (flux_msg_route_first (msg) == NULL && errno == EPROTO,
        "flux_msg_route_first returns NULL errno EPROTO on msg "
        "w/o routes enabled");
    errno = 0;
    ok (flux_msg_route_last (NULL) == NULL && errno == EINVAL,
        "flux_msg_route_last returns -1 errno EINVAL on msg = NULL");
    errno = 0;
    ok (flux_msg_route_last (msg) == NULL && errno == EPROTO,
        "flux_msg_route_last returns NULL errno EPROTO on msg "
        "w/o routes enabled");
    errno = 0;
...
void check_routes (void)
{
    ...
    ok (flux_msg_route_pop (msg) == NULL,
        "flux_msg_route_pop works and sets id to NULL on msg w/o routes");
    ...
    ok ((s = flux_msg_route_pop (msg)) != NULL,
        "flux_msg_route_pop works on msg w/routes");
    like (s, "router",
        "flux_msg_route_pop returns id2 on message with id1+id2");
    free (s);
    ok (flux_msg_route_count (msg) == 1,
        "flux_msg_route_count returns 1 on message w/ id1");

    ok (flux_msg_route_push (msg, "router2") == 0 && flux_msg_frames (msg) == 4,
        "flux_msg_route_push works and adds a frame");
    ok ((flux_msg_route_count (msg) == 2),
        "flux_msg_route_count returns 2 on msg w/ id1+id2");

    ok (flux_msg_route_delete (msg) == 0 && flux_msg_frames (msg) == 3,
        "flux_msg_route_delete works and removed a frame");
    ok (flux_msg_route_count (msg) == 1,
        "flux_msg_route_count returns 1 on message w/ id1");
...
void check_copy (void)
{
    ...
    ok ((s = flux_msg_route_pop (cpy)) != NULL,
        "flux_msg_route_pop pops route from copy");
    like (s, "second",
          "flux_msg_route_pop returns correct second route");
    free (s);

    ok ((s = flux_msg_route_pop (cpy)) != NULL,
        "flux_msg_route_pop pops route from copy");
    like (s, "first",
          "flux_msg_route_pop returns correct first route");
    free (s);
