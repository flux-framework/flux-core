...
void check_routes (void)
{
    ...
    like (s, "sender", "flux_msg_get_route_first returns id on msg w/delim+id");
    free (s);

    ok (flux_msg_get_route_last (msg, &s) == 0 && s != NULL,
        "flux_msg_get_route_last works");
    like (s, "sender", "flux_msg_get_route_last returns id on msg w/delim+id");
    free (s);
...
void check_topic (void)
{
    ...
    ok (flux_msg_push_route (msg, "id1") == 0, "flux_msg_push_route works");
...
void check_sendfd (void)
{
    ...
    ok (pipe2 (pfd, O_CLOEXEC) == 0, "got blocking pipe");
    ok ((msg = flux_msg_create (FLUX_MSGTYPE_REQUEST)) != NULL,
        "flux_msg_create works");
    ok (flux_msg_set_topic (msg, "foo.bar") == 0, "flux_msg_set_topic works");
