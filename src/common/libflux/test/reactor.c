...
static void test_zmq (flux_reactor_t *reactor)
{
    ...
    ok (zs[0] && zs[1] && zsock_bind (zs[0], "%s", uri) == 0
    ...
    ok (r != NULL && w != NULL, "zmq: nonblocking reader and writer created");
    flux_watcher_start (r);
    flux_watcher_start (w);
    ok (flux_reactor_run (reactor, 0) == 0,
        "zmq: reactor ran to completion after %d messages",
        zmqwriter_msgcount);
    flux_watcher_stop (r);
    flux_watcher_stop (w);
...
static void test_fd (flux_reactor_t *reactor)
{
    ...
    ok (r != NULL && w != NULL, "fd: reader and writer created");
    flux_watcher_start (r);
    flux_watcher_start (w);
    ok (flux_reactor_run (reactor, 0) == 0,
        "fd: reactor ran to completion after %lu bytes",
        fdwriter_bufsize);
    flux_watcher_stop (r);
    flux_watcher_stop (w);
    flux_watcher_destroy (r);
...
static void buffer_read (flux_reactor_t *r, flux_watcher_t *w, int revents, void *arg)
{
    ...
    if (revents & FLUX_POLLERR) {
        ...
        ok (len == 6, "buffer: read returned correct length");
...
static void buffer_read_line (flux_reactor_t *r,
                              flux_watcher_t *w,
                              int revents,
                              void *arg)
{
    ...
    if (revents & FLUX_POLLERR) {
        ...
        ok (false,
            "buffer: read line callback failed to return FLUX_POLLIN: %d",
            revents);
    }
...
static void buffer_write (flux_reactor_t *r, flux_watcher_t *w, int revents, void *arg)
{
    ...
    if (revents & FLUX_POLLERR) {
        ok (false, "buffer: write callback called with FLUX_POLLERR");
    } else {
...
static void buffer_read_fill (flux_reactor_t *r,
                              flux_watcher_t *w,
                              int revents,
                              void *arg)
{
...
static void buffer_read_overflow (flux_reactor_t *r,
                                  flux_watcher_t *w,
                                  int revents,
                                  void *arg)
{
    ...
    if (revents & FLUX_POLLERR) {
        ...
        ok (len == 6, "buffer overflow test: read returned correct length");
...
static void test_buffer (flux_reactor_t *reactor)
{
    ...
    ok (write (fd[1], "foobar", 6) == 6, "buffer: write to socketpair success");
    ...
    w = flux_buffer_read_watcher_create (reactor,
                                         ...
                                         &count);
    ok (w != NULL, "buffer: read line created");
    ...
    w = flux_buffer_write_watcher_create (reactor,
                                          ...
                                          0,
                                          &count);
    ok (w != NULL, "buffer: write created");
    ...
    ok (read (fd[1], buf, 1024) == 6, "buffer: read from socketpair success");
    ...
    w = flux_buffer_read_watcher_create (reactor,
                                         ...
                                         0,
                                         &count);
    ok (w != NULL, "buffer overflow test: read line created");

    fb = flux_buffer_read_watcher_get_buffer (w);

    ok (fb != NULL, "buffer overflow test: buffer retrieved");
    ...
    count = 0;
    ok (pipe (pfds) == 0, "buffer: hey I can has a pipe!");
    ...
    ok (fd_set_nonblocking (pfds[1]) >= 0, "buffer: fd_set_nonblocking");
    ...
    w = flux_buffer_write_watcher_create (reactor,
                                          ...
                                          0,
                                          &count);
    ok (w != NULL, "buffer: write watcher close: watcher created");
    fb = flux_buffer_write_watcher_get_buffer (w);
...
static void buffer_read_fd_close (flux_reactor_t *r,
                                  flux_watcher_t *w,
                                  int revents,
                                  void *arg)
{
    ...
    if (revents & FLUX_POLLERR) {
        ...
        if (!bfc->count) {
            ...
            ok (len == 6, "buffer corner case: read returned correct length");
...
static void buffer_read_line_fd_close (flux_reactor_t *r,
                                       flux_watcher_t *w,
                                       int revents,
                                       void *arg)
{
    ...
    if (revents & FLUX_POLLERR) {
        ok (false,
            "buffer corner case: read line callback incorrectly called with "
            "FLUX_POLLERR");
    } else if (revents & FLUX_POLLIN) {
        flux_buffer_t *fb = flux_buffer_read_watcher_get_buffer (w);
        const void *ptr;
...
static void test_buffer_corner_case (flux_reactor_t *reactor)
{
    ...
    w = flux_buffer_read_watcher_create (reactor,
                                         ...
                                         0,
                                         &bfc);
    ok (w != NULL, "buffer corner case: read created");

    fb = flux_buffer_read_watcher_get_buffer (w);

    ok (fb != NULL, "buffer corner case: buffer retrieved");
...
static void test_timer (flux_reactor_t *reactor)
{
    ...
    ok (oneshot_runs == 1, "timer: oneshot was executed once (%.3fs)", elapsed);
    oneshot_runs = 0;
    ok (flux_reactor_run (reactor, 0) == 0, "timer: reactor exited normally");
    ok (oneshot_runs == 0, "timer: expired oneshot didn't run");
    ...
    for (i = 0; i < sizeof (t) / sizeof (t[0]); i++) {
        ...
        ok (rc == 0 && oneshot_runs == 1 && elapsed >= t[i],
            "timer: reactor ran %.3fs oneshot at >= time (%.3fs)",
            t[i],
            elapsed);
    }
    flux_watcher_destroy (w);
...
static void test_idle (flux_reactor_t *reactor)
{
    ...
    ok (w != NULL, "created idle watcher");
    flux_watcher_start (w);

    ok (flux_reactor_run (reactor, 0) == 0, "reactor ran successfully");
    ok (idle_count == 42, "idle watcher ran until stopped");
    flux_watcher_destroy (w);
}
...
static void test_prepcheck (flux_reactor_t *reactor)
{
    ...
    ok (prepare_count >= 8, "prepare watcher ran at least once per timer");
    ok (check_count >= 8, "check watcher ran at least once per timer");
...
static void test_signal (flux_reactor_t *reactor)
{
    ...
    ok (w != NULL, "created signal watcher");
    flux_watcher_start (w);

    idle = flux_idle_watcher_create (reactor, sigidle_cb, NULL);
    ok (idle != NULL, "created idle watcher");
...
static void child_cb (flux_reactor_t *r, flux_watcher_t *w, int revents, void *arg)
{
    ...
    ok (pid == child_pid, "child watcher called with expected rpid");
...
static void test_child (flux_reactor_t *reactor)
{
    ...
    ok (w != NULL, "created child watcher");

    ok (kill (child_pid, SIGHUP) == 0, "sent child SIGHUP");
    flux_watcher_start (w);
...
static void test_stat (flux_reactor_t *reactor)
{
    ...
    ok (ctx.fd >= 0, "created temporary file");
    w = flux_stat_watcher_create (reactor, ctx.path, 0., stat_cb, NULL);
    ok (w != NULL, "created stat watcher");
    flux_watcher_start (w);

    tw = flux_timer_watcher_create (reactor, 0.01, 0.01, stattimer_cb, &ctx);
    ok (tw != NULL, "created timer watcher");
    flux_watcher_start (tw);

    ok (flux_reactor_run (reactor, 0) == 0, "reactor ran successfully");

    tap_skip (stat_size == 1, "stat watcher invoked once for size chnage");
    ok (stat_nlink == 1, "stat watcher invoked once for nlink set to zero");
...
int main (int argc, char *argv[])
{
    ...
    ok ((reactor = flux_reactor_create (0)) != NULL, "created reactor");
    if (!reactor)
