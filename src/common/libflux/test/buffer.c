...
void corner_case (void)
{
    ...
    ok (flux_buffer_size (fb) < 0
        && errno == EINVAL,
        "flux_buffer_size fails on destroyed fb pointer");
    ok (flux_buffer_bytes (fb) < 0
        && errno == EINVAL,
    ...
    ok (flux_buffer_set_read_line_cb (fb, empty_cb, NULL) < 0
        && errno == EINVAL,
        "flux_buffer_set_read_line_cb fails on destroyed fb pointer");
    ok (flux_buffer_set_high_write_cb (fb, empty_cb, 0, NULL) < 0
        && errno == EINVAL,
        "flux_buffer_set_high_write_cb fails on destroyed fb pointer");
