...
void basic (void)
{
    ...
    ok (pipe (pipefds) == 0, "pipe succeeded");
    ...
    ok (flux_buffer_bytes (fb) == 0, "flux_buffer_bytes initially returns 0");
    ...
    ok (flux_buffer_write (fb, "foo", 3) == 3, "flux_buffer_write works");
    ...
    ok ((ptr = flux_buffer_peek (fb, 2, &len)) != NULL && len == 2,
        "flux_buffer_peek with specific length works");

    ok (!memcmp (ptr, "fo", 2), "flux_buffer_peek returns expected data");

    ok ((ptr = flux_buffer_peek (fb, -1, &len)) != NULL && len == 3,
        "flux_buffer_peek with length -1 works");

    ok (!memcmp (ptr, "foo", 3), "flux_buffer_peek returns expected data");

    ok (flux_buffer_bytes (fb) == 3,
        "flux_buffer_bytes returns unchanged length after peek");

    ok (flux_buffer_drop (fb, 2) == 2, "flux_buffer_drop works");
    ...
    ok ((ptr = flux_buffer_read (fb, 2, &len)) != NULL && len == 2,
    ...
    memset (buf, '\0', 1024);
    ok (read (pipefds[0], buf, 1024) == 2, "read correct number of bytes");

    ok (memcmp (buf, "fo", 2) == 0, "read returned correct data");
    ...
    memset (buf, '\0', 1024);
    ok (read (pipefds[0], buf, 1024) == 3, "read correct number of bytes");

    ok (memcmp (buf, "foo", 3) == 0, "read returned correct data");
    ...
    memset (buf, '\0', 1024);
    ok (read (pipefds[0], buf, 1024) == 1, "read correct number of bytes");

    ok (memcmp (buf, "o", 1) == 0, "read returned correct data");
    ...
    ok (write (pipefds[1], "foo", 3) == 3, "write to pipe works");
...
void read_cb (flux_buffer_t *fb, void *arg)
{
    ...
    ok ((ptr = flux_buffer_read (fb, -1, &len)) != NULL && len == 6,
        "flux_buffer_read in callback works");

    ok (!memcmp (ptr, "foobar", 6), "read in callback returns expected data");
}
...
void basic_callback (void)
{
    ...
    ok (count == 1, "read_cb not called again, because not above low mark");
    ...
    ok (flux_buffer_drop (fb, -1) == 6, "flux_buffer_drop cleared all data");
    ...
    ok (count == 0, "read_line_cb not called, no line written yet");
    ...
    ok (count == 1, "read_line_cb called");
    ...
    ok (write (pipefds[1], "foobar", 6) == 6, "write to pipe works");
    ...
    ok ((ptr = flux_buffer_read (fb, 3, &len)) != NULL && len == 3,
        "flux_buffer_read success");

    ok (!memcmp (ptr, "foo", 3), "flux_buffer_read returns expected data");

    ok (count == 0, "write_cb not called, not less than high");

    ok ((ptr = flux_buffer_read (fb, 3, &len)) != NULL && len == 3,
        "flux_buffer_read success");

    ok (!memcmp (ptr, "bar", 3), "flux_buffer_read returns expected data");

    ok (count == 1, "write_cb called");
    ...
    ok (!memcmp (ptr, "a", 1), "flux_buffer_read returns expected data");

    ok (count == 0, "write_cb cleared successfully");
    ...
    ok (flux_buffer_drop (fb, 3) == 3, "flux_buffer_drop success");

    ok (count == 0, "write_cb not called, not less than high");

    ok (flux_buffer_drop (fb, 1) == 1, "flux_buffer_drop success");
    ...
    ok (flux_buffer_drop (fb, -1) == 2, "flux_buffer_drop success");
...
void read_multiple_cb (flux_buffer_t *fb, void *arg)
{
    ...
    ok (len == 3, "read in callback returns correct length");

    ok (!memcmp (ptr, "foo", 3), "read in callback returns expected data");
}
...
void multiple_callback (void)
{
    ...
    ok (count == 3, "read_line_cb called three times");
...
void disable_read_cb (flux_buffer_t *fb, void *arg)
{
    ...
    ok (!flux_buffer_set_low_read_cb (fb, NULL, 0, NULL),
...
void disable_read_line_cb (flux_buffer_t *fb, void *arg)
{
    ...
    ok (!flux_buffer_set_read_line_cb (fb, NULL, NULL),
...
void disable_write_cb (flux_buffer_t *fb, void *arg)
{
    ...
    ok (!flux_buffer_set_high_write_cb (fb, NULL, 0, NULL),
...
void disable_callback (void)
{
    ...
    ok (count == 1, "disable_read_cb not called again, callback is disabled");
    ...
    ok (flux_buffer_write (fb, "foofoo", 6) == 6, "flux_buffer_write success");
    ...
    ok (count == 1, "disable_write_cb called correct number of times");

    ok ((ptr = flux_buffer_read (fb, 3, &len)) != NULL && len == 3,
        "flux_buffer_read success");

    ok (count == 1, "disable_write_cb not called again, successfully disabled");
...
void corner_case (void)
{
    ...
    ok (flux_buffer_create (-1) == NULL && errno == EINVAL,
        "flux_buffer_create fails on bad input -1");

    ok (flux_buffer_create (0) == NULL && errno == EINVAL,
        "flux_buffer_create fails on bad input 0");

    /* all functions fail on NULL fb pointer */
    ok (flux_buffer_size (NULL) < 0 && errno == EINVAL,
        "flux_buffer_size fails on NULL pointer");
    ok (flux_buffer_bytes (NULL) < 0 && errno == EINVAL,
        "flux_buffer_bytes fails on NULL pointer");
    ok (flux_buffer_space (NULL) < 0 && errno == EINVAL,
        "flux_buffer_space fails on NULL pointer");
    ok (flux_buffer_readonly (NULL) < 0 && errno == EINVAL,
    ...
    ok (flux_buffer_peek (NULL, -1, NULL) == NULL && errno == EINVAL,
        "flux_buffer_peek fails on NULL pointer");
    ok (flux_buffer_read (NULL, -1, NULL) == NULL && errno == EINVAL,
        "flux_buffer_read fails on NULL pointer");
    ok (flux_buffer_write (NULL, NULL, 0) < 0 && errno == EINVAL,
        "flux_buffer_write fails on NULL pointer");
    ok (flux_buffer_lines (NULL) < 0 && errno == EINVAL,
        "flux_buffer_lines fails on NULL pointer");
    ok (flux_buffer_drop_line (NULL) < 0 && errno == EINVAL,
        "flux_buffer_drop_line fails on NULL pointer");
    ok (flux_buffer_peek_line (NULL, NULL) == NULL && errno == EINVAL,
        "flux_buffer_peek_line fails on NULL pointer");
    ok (flux_buffer_peek_trimmed_line (NULL, NULL) == NULL && errno == EINVAL,
        "flux_buffer_peek_trimmed_line fails on NULL pointer");
    ok (flux_buffer_read_line (NULL, NULL) == NULL && errno == EINVAL,
        "flux_buffer_read_line fails on NULL pointer");
    ok (flux_buffer_read_trimmed_line (NULL, NULL) == NULL && errno == EINVAL,
        "flux_buffer_read_trimmed_line fails on NULL pointer");
    ok (flux_buffer_write_line (NULL, "foo") < 0 && errno == EINVAL,
        "flux_buffer_write_line fails on NULL pointer");
    ok (flux_buffer_peek_to_fd (NULL, 0, 0) < 0 && errno == EINVAL,
        "flux_buffer_peek_to_fd fails on NULL pointer");
    ok (flux_buffer_read_to_fd (NULL, 0, 0) < 0 && errno == EINVAL,
        "flux_buffer_read_to_fd fails on NULL pointer");
    ok (flux_buffer_write_from_fd (NULL, 0, 0) < 0 && errno == EINVAL,
    ...
    ok (flux_buffer_write (fb, NULL, 0) < 0 && errno == EINVAL,
        "flux_buffer_write fails on bad input");
    ok (flux_buffer_write (fb, "foo", -1) < 0 && errno == EINVAL,
        "flux_buffer_write fails on bad input");
    ok (flux_buffer_write_line (fb, NULL) < 0 && errno == EINVAL,
        "flux_buffer_write_line fails on bad input");
    ok (flux_buffer_write_from_fd (fb, -1, 0) < 0 && errno == EINVAL,
...
void full_buffer (void)
{
    ...
    ok ((fb = flux_buffer_create (4)) != NULL, "flux_buffer_create works");

    ok (flux_buffer_write (fb, "1234", 4) == 4, "flux_buffer_write success");
    ...
    ok (flux_buffer_drop (fb, -1) == 4, "flux_buffer_drop works");

    ok (flux_buffer_write_line (fb, "1234") < 0 && errno == ENOSPC,
...
void readonly_buffer (void)
{
    ...
    ok (flux_buffer_write (fb, "foobar", 6) < 0 && errno == EROFS,
        "flux_buffer_write fails b/c readonly is set");

    ok (flux_buffer_write_line (fb, "foobar") < 0 && errno == EROFS,
        "flux_buffer_write_line fails b/c readonly is set");

    ok (write (pipefds[1], "foo", 3) == 3, "write to pipe works");

    ok (flux_buffer_write_from_fd (fb, pipefds[0], -1) < 0 && errno == EROFS,
