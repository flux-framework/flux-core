...
static void test_composite_basic_any (flux_reactor_t *r, bool with_error)
{
    ...
    ok (rc == 0, "flux_future_push (any, 'f1', f1) == %d", rc);
    ...
    ok (!flux_future_is_ready (f1), "future f1 is not ready");
    ok (flux_future_is_ready (f2), "future f2 is ready");
...
static void test_composite_basic_all (flux_reactor_t *r, bool with_error)
{
    ...
    ok (rc == 0, "flux_future_push (all, 'f1', f1) == %d", rc);
    ...
    ok (!flux_future_is_ready (all), "wait_all future still not ready");
...
static void test_basic_chained (flux_reactor_t *r)
{
    ...
    ok (flux_future_get (f3, NULL) == 0, "chained: flux_future_get == 0");
    ok (flux_future_aux_get (f3, "test_aux") == (void *)0x42,
        "chained: aux item set in prev future available in chained future");
    is (str, "step1-step2-step3", "chained: futures ran in correct order");
    ...
    is (str, "step1", "chained: no chained callbacks run by default on error");
    ...
    flux_future_set_reactor (f, r);
    ok (!flux_future_is_ready (f3), "chained (failure): future not ready");

    flux_future_fulfill (f, NULL, NULL);

    ok (flux_future_wait_for (f3, 0.1) == 0,
        "chained (failure): flux_future_wait_for finished");
    ok (flux_future_is_ready (f3), "chained (failure): flux_future_is_ready");
...
void async_check (flux_future_t *fc, void *arg)
{
    bool with_error = *(bool *)arg;
    flux_future_t *f;
    ok (flux_future_is_ready (fc) == true, "async: composite future is ready");
    if (with_error) {
...
void test_composite_all_async (bool with_error)
{
    ...
    ok (flux_future_push (fc, "f1", f) == 0, "flux_future_push success");
    ...
    ok (flux_future_is_ready (fc) == 0, "flux_future_is_ready returns false");
    ok (flux_reactor_run (r, 0) == 0, "flux_reactor_run returned");
    ok (async_check_rc == 0, "asynchronous callback called");
...
void test_composite_any_async (bool with_error)
{
    ...
    ok (count >= 0, "flux_reactor_run returned %d", count);
    ok (async_any_check_rc == 0, "asynchronous callback called");
...
void chained_async_cb (flux_future_t *f, void *arg)
{
    ...
    ok (flux_future_is_ready (f), "chained_async_cb: future is ready");
    ok (flux_future_get (f, (const void **)&result) == 0,
        "chained_async_cb: flux_future_get worked");
    is (result, expected, "chained_async_cb: got expected result");
    flux_future_destroy (f);
}
...
void test_chained_async ()
{
    ...
    ok (flux_reactor_run (r, 0) == 0, "chained async: reactor exited");
    flux_reactor_destroy (r);
}
...
void test_chained_multiple_fulfill ()
{
    ...
    ok (*rp == 3, "multiple fulfill: flux_future_get returned %d", *rp);
    ok (count == 4, "multiple fulfill: continuation called %d times", count);
    flux_future_destroy (f);
