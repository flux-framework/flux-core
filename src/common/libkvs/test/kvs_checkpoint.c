...
void errors (void)
{
    ...
    errno = 0;
    ok (kvs_checkpoint_lookup_get_checkpoints (NULL, NULL) < 0
        && errno == EINVAL,
        "kvs_checkpoint_lookup_get_checkpoints fails on bad input");
    ...
    errno = 0;
    ok (kvs_checkpoint_lookup_get_checkpoints (f, &checkpoints) < 0
        && errno == EDEADLOCK,
        "kvs_checkpoint_lookup_get_checkpoints fails on unfulfilled future");
