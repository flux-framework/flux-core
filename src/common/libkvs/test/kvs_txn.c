...
void basic (void)
{
    ...
    ok (txn != NULL, "flux_kvs_txn_create works");
    rc = flux_kvs_txn_pack (txn, 0, "foo.bar.baz", "i", 42);
    ok (rc == 0, "1: flux_kvs_txn_pack(i) works");
    rc = flux_kvs_txn_pack (txn, 0, "foo.bar.bleep", "s", "foo");
    ok (rc == 0, "2: flux_kvs_txn_pack(s) works");
    rc = flux_kvs_txn_unlink (txn, 0, "a");
    ok (rc == 0, "3: flux_kvs_txn_unlink works");
    rc = flux_kvs_txn_mkdir (txn, 0, "b.b.b");
    ok (rc == 0, "4: flux_kvs_txn_mkdir works");
    rc = flux_kvs_txn_symlink (txn, 0, "c.c.c", NULL, "b.b.b");
    ok (rc == 0, "5: flux_kvs_txn_symlink works (no namespace)");
    rc = flux_kvs_txn_put (txn, 0, "d.d.d", "43");
    ok (rc == 0, "6: flux_kvs_txn_put(i) works");
    rc = flux_kvs_txn_unlink (txn, 0, "e");
    ok (rc == 0, "7: flux_kvs_txn_unlink works");
    rc = flux_kvs_txn_put (txn, 0, "nerrrrb", NULL);
    ok (rc == 0, "8: flux_kvs_txn_put(NULL) works");
    rc = flux_kvs_txn_symlink (txn, 0, "f.f.f", "g.g.g", "h.h.h");
    ok (rc == 0, "9: flux_kvs_txn_symlink works (namespace)");
    errno = 0;
    ...
    ok (rc < 0 && errno == EINVAL, "error: flux_kvs_txn_mkdir works");
    errno = 0;
...
void test_raw_values (void)
{
    ...
    ok (nlen == 0, "and it is size of zero");
    ok (nbuf == NULL, "and buffer is NULL");
    ...
    ok (memcmp (nbuf, buf, nlen) == 0, "and it is the correct content");
    free (nbuf);
    flux_kvs_txn_destroy (txn);
}
