                      ...
                      int rank)
{
    ...
    rc = pmi_process_mapping_find_ranks (blocks, nblocks, rank, size,
                                         ranks, nranks);
    if (rc != PMI_SUCCESS) {
        free (ranks);
        return NULL;
...
int main (int argc, char *argv[])
{
    ...
    ok (rc == PMI_SUCCESS,
        "parsed cyclic layout of 4 procs on 2 nodes");
    if (rc != PMI_SUCCESS)
    ...
    rc = pmi_process_mapping_find_nranks (blocks, nblocks, 0,
                                          valid[4].size, &nranks);
    ok (rc == PMI_SUCCESS && nranks == 2,
        "find_nranks says node 0 runs two procs");
    ranks = xzmalloc (sizeof (int)*nranks);
    rc = pmi_process_mapping_find_ranks (blocks, nblocks, 0,
                                         valid[4].size, ranks, nranks);
    ok (rc == PMI_SUCCESS && ranks[0] == 0 && ranks [1] == 2,
        "find_ranks says node 0 runs 0,2");
    free (ranks);

    rc = pmi_process_mapping_find_nranks (blocks, nblocks, 1,
                                          valid[4].size, &nranks);
    ok (rc == PMI_SUCCESS && nranks == 2,
        "find_nranks says node 1 runs two procs");
    ranks = xzmalloc (sizeof (int)*nranks);
    rc = pmi_process_mapping_find_ranks (blocks, nblocks, 1,
                                         valid[4].size, ranks, nranks);
    ...
    ok (rc == PMI_SUCCESS,
        "parsed block layout of 4 procs on 2 nodes");
    if (rc != PMI_SUCCESS)
        BAIL_OUT ("cannot continue");
    rc = pmi_process_mapping_find_nodeid (blocks, nblocks, 0, &nodeid);
    ok (rc == PMI_SUCCESS && nodeid == 0,
    ...
    for (i = 0; valid[i].vec != NULL; i++) {
        ...
        ok (rc == PMI_SUCCESS,
            "parsed %s size=%d", valid[i].vec, valid[i].size);
    ...
    for (i = 0; invalid[i].vec != NULL; i++) {
        ...
        ok (rc == PMI_FAIL,
            "refused to parse %s", invalid[i].vec);
    }
