...
int main (int argc, char *argv[])
{
     ...
     * implementation.
     */
    if (unsetenv ("PMI_FD") < 0 || unsetenv ("PMIX_SERVER_URI") < 0
        || unsetenv ("PMIX_SERVER_URI2") < 0 || unsetenv ("PMI_LIBRARY") < 0
        || setenv ("FLUX_PMI_SINGLETON", "1", 1) < 0)
        BAIL_OUT ("Environment setup failed");

    pmi = pmi_wrap_create (INTREE_PMI_LIBRARY_PATH, &ops, false);
    ok (pmi == NULL, "pmi_wrap_create recursion safety check works");

    pmi = pmi_wrap_create ("/noexist", &ops, false);
    ok (pmi == NULL, "pmi_wrap_create fails on wrong library path");
    ...
    ok (rc == PMI_SUCCESS && spawned == 0, "pmi_wrap_init works, spawned = 0");
    initialized = -1;
    ...
    ok (rc == PMI_SUCCESS && size == 1, "pmi_wrap_get_size works, size == 1");
    rank = -1;
    rc = ops->get_rank (pmi, &rank);
    ok (rc == PMI_SUCCESS && rank == 0, "pmi_wrap_get_rank works, rank == 0");
    appnum = -2;
    ...
    ok (rc == PMI_SUCCESS && size == 1, "pmi_wrap_get_universe_size works, size == 1");
    ...
    ok (rc == PMI_SUCCESS && strlen (kvsname) > 0, "pmi_wrap_kvs_get_my_name works");
    ...
    ok (rc == PMI_SUCCESS, "pmi_wrap_kvs_put works");
    rc = ops->kvs_commit (pmi, kvsname);
    ok (rc == PMI_SUCCESS, "pmi_wrap_kvs_commit works");
    rc = ops->barrier (pmi);
    ok (rc == PMI_SUCCESS, "pmi_wrap_barrier works");
    rc = ops->kvs_get (pmi, kvsname, "foo", kvsval, kvsval_length);
    ok (rc == PMI_SUCCESS && !strcmp (kvsval, "bar"), "pmi_wrap_kvs_get works ");
    ...
    ok (rc == PMI_FAIL, "pmi_wrap_publish_name fails with PMI_FAIL");
    rc = ops->unpublish_name (pmi, "foo");
    ok (rc == PMI_FAIL, "pmi_wrap_unpublish_name fails with PMI_FAIL");
    rc = ops->lookup_name (pmi, "foo", port);
    ok (rc == PMI_FAIL, "pmi_wrap_lookup_name fails with PMI_FAIL");

    rc = ops->spawn_multiple (pmi, 0, NULL, NULL, NULL, NULL, NULL, 0, NULL, NULL);
    ok (rc == PMI_FAIL, "pmi_wrap_spawn_multiple fails with PMI_FAIL");

    dies_ok ({ ops->abort (pmi, 0, "a test message"); },
             "pmi_wrap_abort exits program");

    rc = ops->finalize (pmi);
    ok (rc == PMI_SUCCESS, "pmi_wrap_finalize works");
