...
static json_t *eventlog_entry_decode_wrapper (const char *event,
...
static int eventlog_entry_append (json_t *a, const char *event)
{
    ...
    if (!(o = eventlog_entry_decode_wrapper (event, false)))
        return -1;
...
{
    return eventlog_entry_decode_wrapper (entry, true);
}
