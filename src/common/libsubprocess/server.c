 ...
 *   waitable processes (client-side rexec-bg-only flag)
...
static inline bool is_waitable (flux_subprocess_t *p)
{
     ...
     * flag and they are in background mode. In foreground mode (i.e.
     * streaming RPC, the client is already waiting by virtue of the
     * eventual finished response.
     */
    return (p->bg && p->flags & FLUX_SUBPROCESS_FLAGS_WAITABLE);
}
