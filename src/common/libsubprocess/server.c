...
static void proc_end_stream (subprocess_server_t *s,
                             const flux_msg_t *request,
                             flux_subprocess_t *p)
{
    ...
    /* If this is a background process, remove request from subprocess aux
     * item list, since it will no longer be vailid after this point.
     */
    if (p->bg)
                                  ...
                                  flux_subprocess_state_t state)
{
    ...
    if (state == FLUX_SUBPROCESS_RUNNING) {
        ...
        /* End stream of RPC responses if this process is run in background
         */
        if (p->bg)
            proc_end_stream (s, request, p);
    }
        ...
        /* N.B. background process may also fail here */
...
static void proc_output_cb (flux_subprocess_t *p, const char *stream)
{
    ...
    if (!p->bg) {
        ...
        if (streq (stream, "stderr"))
            llog_error (s, "%s: %s", command, buf);
        else
            llog_info (s, "%s: %s", command, buf);
    }
    return;
                            ...
                            void *arg)
{
    ...
    if (!(flags & (SUBPROCESS_REXEC_STDOUT | SUBPROCESS_REXEC_BACKGROUND)))
        ops.on_stdout = NULL;
    if (!(flags & (SUBPROCESS_REXEC_STDERR | SUBPROCESS_REXEC_BACKGROUND)))
        ops.on_stderr = NULL;
    ...
    if (flags & SUBPROCESS_REXEC_BACKGROUND)
        p->bg = true;
