...
/* very similar to above test but we send initial write, and there
 * should be one fewer credit callback
 */
void test_on_credit_no_initial_credits (flux_reactor_t *r)
{
    char *av[] = { TEST_SUBPROCESS_DIR "test_echo", "-O",  NULL };
    flux_cmd_t *cmd;
    flux_subprocess_t *p = NULL;
    int credits = 8;            /* start at 8 b/c we're using NO_INITIAL_CREDITS */
    int ret;
    ...
    ok (completion_cb_count == 1, "completion callback called 1 time");
    ok (stdout_output_cb_count >= 2, "stdout output callback called >= 2 times");
    ok (credit_cb_count == 5, "credit callback called 5 times");
    flux_subprocess_destroy (p);
    flux_cmd_destroy (cmd);
}
...
int main (int argc, char *argv[])
{
    ...
    diag ("on_credit");
    test_on_credit (r);
    diag ("on_credit_no_initial_credits");
    test_on_credit_no_initial_credits (r);
