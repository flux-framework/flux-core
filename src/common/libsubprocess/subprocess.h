...
enum {
    ...
    /* flux_rexec_bg() only: allow flux_rexec_wait(3) to be called
     * for this subprocess
     */
 ...
 * If service_name is NULL, then default to `rexec`.
 *
 * The future will be fulfilled with the payload ``{"status":i}`` on process
 * completion, or with an error response if there is an error.
 */
