...
void test_basic (void)
{
     ...
     */
    cf = cf_create ();
    ok (cf != NULL, "cf_create works");
    ok (cf_typeof (cf) == CF_TABLE, "cf_typeof says empty cf is CF_TABLE");

    /* Copy a cf object.
     */
    cf_cpy = cf_copy (cf);
    ok (cf_cpy != NULL, "cf_copy works");
    ok (cf_typeof (cf) == CF_TABLE, "cf_typeof says copy is CF_TABLE");
    cf_destroy (cf_cpy);

    /* Read some TOML into the cf top level table
     */
    rc = cf_update (cf, t1, strlen (t1), &error);
    ok (rc == 0, "cf_update t1 worked");
     ...
     */
    ok (cf_int64 (cf_get_in (cf, "i")) == 1, "accessed int64 value");
    ok (cf_double (cf_get_in (cf, "d")) == 3.14, "accessed double value");
    s = cf_string (cf_get_in (cf, "s"));
    ok (s != NULL && !strcmp (s, "foo"), "accessed string value");
    ok (cf_bool (cf_get_in (cf, "b")) == true, "accessed bool value");
    t = cf_timestamp (cf_get_in (cf, "ts"));
    ok (t == strtotime ("1979-05-27T07:32:00Z"), "accessed ts value ");
    cf2 = cf_get_in (cf, "ai");

    /* Access array and its elements.
     */
    ok (cf2 && cf_typeof (cf2) == CF_ARRAY, "accessed array");
    ok (cf_array_size (cf2) == 3, "array has expected size");
    ok (cf_int64 (cf_get_at (cf2, 0)) == 1 && cf_int64 (cf_get_at (cf2, 1)) == 2
...
void test_corner (void)
{
    ...
    ok (cf_update (cf, junk, strlen (junk), &error) < 0 && errno == EINVAL,
        "cf_update buf=\"%s\" fails with EINVAL",
        junk);
    ...
    /* cf_typeof
     */
    ok (cf_typeof (NULL) == CF_UNKNOWN, "cf_typeof cf=NULL returns CF_UNKNOWN");

    /* simple accessors
     */
    ok (cf_int64 (NULL) == 0, "cf_int64 cf=NULL returns 0");
    ok (cf_double (NULL) == 0., "cf_double cf=NULL returns 0.");
    ok (strlen (cf_string (NULL)) == 0, "cf_string cf=NULL returns \"\"");
    ok (cf_bool (NULL) == false, "cf_bool cf=NULL returns false");
    ok (cf_timestamp (NULL) == 0, "cf_timestamp cf=NULL returns 0");

    /* cf_array_size
     */
    ok (cf_array_size (NULL) == 0, "cf_array_size cf=NULL returns 0");
    ok (cf_array_size (cf) == 0, "cf_array_size cf=(not array) returns 0");

    cf_destroy (cf);
}

const struct cf_option opts_extra[] = {
    // for 't1'
    // { "i", CF_INT64, true },
    {"d", CF_DOUBLE, true},
    {"s", CF_STRING, true},
...
const struct cf_option opts_missing[] = {
    // for 't1'
    {"i", CF_INT64, true},
    {"d", CF_DOUBLE, true},
    {"s", CF_STRING, true},
...
const struct cf_option opts_optional[] = {
    // for 't1'
    {"i", CF_INT64, true},
    {"d", CF_DOUBLE, true},
    {"s", CF_STRING, true},
...
const struct cf_option opts_wrongtype[] = {
    // for 't1'
...
static void create_test_file (const char *dir,
                              char *prefix,
                              char *path,
                              size_t pathlen,
                              const char *contents)
...
void test_update_file (void)
{
    ...
    ok (cf_update_file (cf, path, &error) == 0, "cf_update_file works");
    errno = 0;
...
void test_update_glob (void)
{
    ...
    ok ((cf2 = cf_get_in (cf, "tab2")) != NULL, "found tab2 table in cf");

    rc = cf_check (cf2, opts_multi, CF_STRICT, &error);
    ok (rc == 0, "cf_check tab2 worked");
    cfdiag (rc, "cf_check tab2", &error);

    ok ((cf3 = cf_get_in (cf, "tab3")) != NULL, "found tab3 table in cf");
    ...
    if ((unlink (path1) < 0) || (unlink (path2) < 0) || (unlink (path3) < 0)
        || (unlink (invalid) < 0))
...
void test_check (void)
{
    ...
    ok (rc == 0, "cf_check flags=0 allows extra key");
    ...
    ok (rc == 0, "cf_check succeeds on missing optional key");
    cfdiag (rc, "cf_check", &error);
    ...
    ok (rc < 0 && errno == EINVAL, "cf_check fails on wrong type");
    cfdiag (rc, "cf_check", &error);
