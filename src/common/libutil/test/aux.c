...
void aux_destroy_set_ok (void)
{
    ...
    aux_destroy (&ac.aux);
    ok (ac.count == 3, "aux_destroy allows list to be modified");
...
void simple_test (void)
{
    ...
    myfree_count = 0;
    aux_destroy (&aux);
    cmp_ok (myfree_count, "==", 2, "aux_destroy called myfree twice");
    ok (aux == NULL, "aux_destroy set aux to NULL");

    lives_ok ({ aux_destroy (NULL); }, "aux_destroy aux=NULL doesn't crash");
}
