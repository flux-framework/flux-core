...
void test_basic (void)
{
    ...
    ok (idset != NULL, "idset_create size=0 works");
...
void test_codec_large (void)
{
    ...
    ok (idset != NULL, "idset_decode '0-5000' works");
    ...
    ok (count == 5001, "idset_encode flags=0x0 '0,2,3,...,5000' works");
    if (count != 5001)
        diag ("count=%d", count);
    free (s);
    idset_destroy (idset);
...
void test_badparam (void)
{
    ...
    errno = 0;
    ok (idset_clear (idset, 101) == 0, "iset_clear(id=out of range) works");
    errno = 0;
    ...
    ok (idset_test (NULL, 1) == false, "iset_test(idset=NULL) returns false");

    ok (idset_count (NULL) == 0, "iset_count(idset=NULL) returns 0");
...
void test_iter (void)
{
    ...
    ok (idset_first (idset) == 7, "idset_first idset=[7-9] returned 7");
    ok (idset_next (idset, 7) == 8, "idset_next idset=[7-9] prev=7 returned 8");
    ok (idset_next (idset, 8) == 9, "idset_next idset=[7-9] prev=8 returned 9");
    ...
    ok (idset_last (idset) == 9, "idset_last idset=[7-9] returned 9");
...
void test_set (void)
{
    ...
    ok (idset_count (idset) == 0, "idset_count (idset) == 0");
    ok (idset_set (idset, 0) == 0, "idset_set 0 worked");
    ok (idset_count (idset) == 1, "idset_count (idset) == 1");
    ok (idset_set (idset, 0) == 0, "idset_set 0 again  succeeds");
    ok (idset_count (idset) == 1, "idset_count (idset) == 1");
    ok (idset_set (idset, 3) == 0, "idset_set 3 worked");
    ok (idset_set (idset, 99) == 0, "idset_set 99 worked");
    errno = 0;
    ...
    ok (idset_first (idset) == 0, "idset_first returned 0");
    ok (idset_next (idset, 0) == 3, "idset_next prev=0 returned 3");
    ok (idset_next (idset, 3) == 99, "idset_next prev=3 returned 99");
...
void test_range_set (void)
{
    ...
    ok (idset_range_set (idset, 0, 2) == 0, "idset_range_set 0-2 worked");
    ok (idset_count (idset) == 3, "idset_count == 3");
    ok (idset_range_set (idset, 0, 2) == 0, "idset_range_set 0-2 again worked");
    ok (idset_count (idset) == 3, "idset_count == 3");
    ok (idset_range_set (idset, 80, 79) == 0,  // reversed
        "idset_set 80-79 worked");
    ok (idset_count (idset) == 5, "idset_count == 5");
    ...
    errno = 0;
    ok (idset_range_set (idset, IDSET_INVALID_ID, IDSET_INVALID_ID + 1) < 0
            && errno == EINVAL,
        "idset_set id=INVALID-(INVALID+1) failed with EINVAL");

    ok (idset_first (idset) == 0, "idset_first returned 0");
    ok (idset_next (idset, 0) == 1, "idset_next prev=0 returned 1");
    ok (idset_next (idset, 1) == 2, "idset_next prev=1 returned 2");
    ok (idset_next (idset, 2) == 79, "idset_next prev=2 returned 79");
    ok (idset_next (idset, 79) == 80, "idset_next prev=2 returned 80");
...
void test_clear (void)
{
    ...
    ok (idset_count (idset) == 10, "idset_count [1-10] returns 10");
    for (id = 1; id <= 7; id++) {
        ok (idset_test (idset, id) == true, "idset_test %d initially true", id);
        ok (idset_clear (idset, id) == 0,
            "idset_clear idset=[%d-10], id=%d worked",
            id,
            id);
        ok (idset_test (idset, id) == false, "idset_test %d is now false", id);
    }
...
void test_range_clear (void)
{
    ...
    ok (idset_first (idset) == 1, "idset_first returned 1");
    ok (idset_next (idset, 1) == 10, "idset_next prev=1 returned 10");
    ok (idset_next (idset, 10) == IDSET_INVALID_ID,
...
void test_equal (void)
{
    ...
    ok (idset_equal (set1, set2) == false, "idset_equal (NULL, NULL) == false");

    if (!(set1 = idset_decode ("1-10")))
        BAIL_OUT ("idset_decode [1-10] failed");
    ok (idset_equal (set1, set2) == false, "idset_equal (set1, NULL) == false");
...
void test_copy (void)
{
    ...
    ok (idset_count (idset) == 5000, "idset_count idset=[1-5000] returns 5000");
    ok ((cpy = idset_copy (idset)) != NULL, "idset_copy made a copy");
    ok (idset_count (cpy) == 5000, "idset_count on copy returns 5000");
    ok (idset_equal (idset, cpy) == true, "idset_copy made an accurate copy");
    ok (idset_clear (cpy, 100) == 0, "idset_clear 100 on copy");
    ok (idset_count (cpy) == 4999, "idset_count on copy returns 4999");
    ok (idset_count (idset) == 5000, "idset_count on orig returns 5000");
    idset_destroy (cpy);
...
void test_autogrow (void)
{
    ...
    ok (idset != NULL, "idset_create size=1 flags=0 works");
    ...
    ok (idset != NULL, "idset_create size=1 flags=AUTOGROW works");
    ok (idset->T.M == 1, "idset internal size is 1");
    ok (idset_set (idset, 0) == 0, "idset_set 0 works");
    ok (idset_set (idset, 2) == 0, "idset_set 2 works");
    ok (idset->T.M > 1, "idset internal size grew");
    ok (idset_test (idset, 0) && !idset_test (idset, 1) && idset_test (idset, 2)
            && !idset_test (idset, 3),
