...
"    -H,--handle-path PATH prepend PATH to handle search path\n"
...
int main (int argc, char *argv[])
{
    ...
    setup_handle_env (cf, Hopt);    /* sets FLUX_HANDLE_PATH */
...
void setup_handle_env (flux_conf_t cf, const char *path_add)
{
    const char *cf_path = flux_conf_get (cf, "general.handle_path");
    char *path = NULL;

    path_push (&path, HANDLE_PATH, ":");

    if (cf_path)
        path_push (&path, cf_path, ":");
    if (path_add)
        path_push (&path, path_add, ":");
    if (setenv ("FLUX_HANDLE_PATH", path, 1) < 0)
        err_exit ("%s", path);
...
void dump_environment (void)
{
    ...
    dump_environment_one ("FLUX_HANDLE_PATH");
