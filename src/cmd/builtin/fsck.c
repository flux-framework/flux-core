...
struct fsck_ctx {
    ...
    json_t *root;
    int sequence;
    json_t *lost_and_found_dir;
    int lost_and_found_count;
    zlist_t *lost_and_found_treeobjs;
    char *hash_name;
    bool verbose;
    bool quiet;
    bool repair;
...
static void put_valref_lost_and_found (struct fsck_ctx *ctx,
                                       ...
                                       json_t *repaired)
{
    json_t *dir = ctx->lost_and_found_dir;
    char *cpy;
    char *next, *name;
    ...
    if (!ctx->quiet)
        fprintf (stderr, "%s repaired and put in lost+found\n", path);

    ctx->lost_and_found_count++;
}
                         ...
                         json_t *treeobj)
{
    ...
    if (fvd.errorcount) {
        ...
        if (ctx->repair
            ...
            /* create lost+found dir if it doesn't exist yet */
            if (!ctx->lost_and_found_dir)
                ctx->lost_and_found_dir = get_dir (ctx,
                                                   ctx->root,
                                                   "lost+found");

            repaired = repair_valref (ctx, treeobj, &fvd);
...
static void save_lost_and_found_treeobj (struct fsck_ctx *ctx,
                                         json_t *treeobj)
{
    if (zlist_append (ctx->lost_and_found_treeobjs,
                      json_incref (treeobj)) < 0)
        log_err_exit ("failed to save treeobj");
    zlist_freefn (ctx->lost_and_found_treeobjs,
                  treeobj,
                  (zlist_free_fn *) json_decref,
                  true);
}

static void get_lost_and_found_treeobjs (struct fsck_ctx *ctx, json_t *dir)
{
    json_t *dir_data;
    void *iter;
    ...
    while (iter) {
        ...
        if (treeobj_is_dir (dir_entry)) {
            ...
            get_lost_and_found_treeobjs (ctx, dir_entry);

            save_lost_and_found_treeobj (ctx, dir_entry);
...
static void store_lost_and_found_treeobjs (struct fsck_ctx *ctx)
{
    json_t *obj;

    /* N.B. future work, parallelize this */
    while ((obj = zlist_pop (ctx->lost_and_found_treeobjs))) {
        char *data;
        ssize_t datalen;
        flux_future_t *f;
...
static int cmd_fsck (optparse_t *p, int ac, char *av[])
{
    ...
    if (ctx.repair && ctx.lost_and_found_count) {
        const char *tmp;
        if (!(tmp = flux_attr_get (ctx.h, "content.hash"))
            || !(ctx.hash_name = strdup (tmp)))
            log_err_exit ("could not get content hash name");

        if (!(ctx.lost_and_found_treeobjs = zlist_new ()))
            log_err_exit ("cannot create list for treeobjs");

        get_lost_and_found_treeobjs (&ctx, ctx.root);

        /* and gotta save root at end too */
        save_lost_and_found_treeobj (&ctx, ctx.root);

        store_lost_and_found_treeobjs (&ctx);

        sync_checkpoint (&ctx);
    }

    zlist_destroy (&ctx.lost_and_found_treeobjs);
    json_decref (ctx.lost_and_found_dir);
