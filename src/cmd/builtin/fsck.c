...
static void output_verrmsg (const char *fmt, va_list ap)
{
    char buf[128];
...
void output_errmsg (struct fsck_ctx *ctx, const char *fmt, ...)
{
    va_list ap;
    if (ctx->quiet)
        return;
    va_start (ap, fmt);
    output_verrmsg (fmt, ap);
    va_end (ap);
}
    ...
    if (fvd.errorcount) {
        ...
        if (!ctx->verbose) {
            if (fvd.errnum == ENOENT)
                output_errmsg (ctx, "%s: missing blobref(s)", path);
            else
                output_errmsg (ctx,
        ...
        if (ctx->repair
            ...
            output_errmsg (ctx,
                           "%s repaired and moved to lost+found",
                           path);
                         ...
                         json_t *treeobj)
{
    ...
    if (!(f = content_load_byblobref (ctx->h,
        ...
        if (errno == ENOENT)
            output_errmsg (ctx, "%s: missing dirref blobref", path);
        else
            output_errmsg (ctx,
        ...
        if (ctx->repair && errno == ENOENT) {
            unlink_path (ctx, path);
            output_errmsg (ctx,
                           "%s unlinked due to missing blobref",
                           path);
        }
        flux_future_destroy (f);
        return;
    ...
    if (!(treeobj_deref = treeobj_decodeb (buf, buflen))) {
        output_errmsg (ctx, "%s: could not decode directory", path);
        ctx->errorcount++;
        goto cleanup;
    }
    if (!treeobj_is_dir (treeobj_deref)) {
        output_errmsg (ctx, "%s: dirref references non-directory", path);
        ctx->errorcount++;
        goto cleanup;
    }
                          ...
                          json_t *treeobj)
{
    if (treeobj_validate (treeobj) < 0) {
        output_errmsg (ctx, "%s: invalid tree object", path);
        ctx->errorcount++;
        return;
    }
    if (ctx->verbose)
        output_errmsg (ctx, "%s", path);
...
static void sync_checkpoint (struct fsck_ctx *ctx)
{
    ...
    output_errmsg (ctx, "Updated primary checkpoint to include lost+found");
}
...
static int cmd_fsck (optparse_t *p, int ac, char *av[])
{
    ...
    output_errmsg (&ctx, "Total errors: %d", ctx.errorcount);
