...
void submitbench_continuation (flux_future_t *f, void *arg)
{
    ...
    if (flux_job_add_get_id (f, &id) < 0)
        log_err_exit ("%s", flux_rpc_get_error (f));
                     ...
                     int revents, void *arg)
{
    ...
    if (ctx->txcount < ctx->totcount
        ...
        if (!(f = flux_job_add (ctx->h, ctx->J, ctx->flags)))
            log_err_exit ("flux_job_add");
#else
        char *cpy = strndup (ctx->jobspec, ctx->jobspecsz);
        if (!cpy)
            log_err_exit ("strndup");
        if (!(f = flux_job_add (ctx->h, cpy, ctx->flags)))
            log_err_exit ("flux_job_add");
        free (cpy);
#endif
