...
int cmd_link (optparse_t *p, int argc, char **argv)
{
    ...
    if (!(txn = flux_kvs_txn_create ()))
        log_err_exit ("flux_kvs_txn_create");
    if (linkns) {
        if (flux_kvs_txn_nslink (txn, 0, linkname, linkns, target) < 0)
            log_err_exit ("%s", linkname);
    }
    else {
        if (flux_kvs_txn_symlink (txn, 0, linkname, target) < 0)
            log_err_exit ("%s", linkname);
    }
    if (!(f = flux_kvs_commit (h, 0, txn)) || flux_future_get (f, NULL) < 0)
        log_err_exit ("flux_kvs_commit");
...
int cmd_copy (optparse_t *p, int argc, char **argv)
{
    ...
    if (!(f = flux_kvs_copy (h, srckey, srcns, dstkey, dstns, 0))
            || flux_future_get (f, NULL) < 0)
...
int cmd_move (optparse_t *p, int argc, char **argv)
{
    ...
    if (!(f = flux_kvs_move (h, srckey, srcns, dstkey, dstns, 0))
            || flux_future_get (f, NULL) < 0)
