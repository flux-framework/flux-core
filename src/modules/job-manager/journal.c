                           ...
                           json_t *entry)
{
    ...
    if (!(o = json_pack ("{s:I s:[O]}",
                         ...
                         "events", entry)))
        goto error;
    if (streq (name, "submit") || streq (name, "alloc")) {
        struct job *job;
                         ...
                         bool full)
{
    ...
    double t_start = flux_reactor_now (r);
    ...
    if (job_count > 0) {
        ...
        flux_log (ctx->h,
                  LOG_DEBUG,
                  "finished sending journal backlog (%s)",
                  fsd);
    }
    /* Send a special response to demarcate the backlog from ongoing events.
     * Consumer should discard message with id = FLUX_JOBID_ANY.
     */
