...
static void rnode_alloc_and_check (struct rnode *n, int count, char *expected)
{
    ...
    ok (rnode_alloc (n, count, &ids) == 0, "rnode_alloc: count=%d", count);
    ok (ids != NULL, "rnode_alloc: returns non-null idset");
    ...
    is (result,
        expected,
        "rnode_alloc: count=%d: returned expected result %s",
        count,
        result);
...
static void rnode_avail_check (struct rnode *n, const char *expected)
{
    ...
    is (avail, expected, "rnode->avail is expected: %s", avail);
    free (avail);
}
...
int main (int ac, char *av[])
{
    ...
    ok (rnode_avail (n) == 0, "rnode_avail still is 0");
    rnode_avail_check (n, "");

    ok (rnode_free (n, "0-1") == 0, "rnode_free (0-1) works");
    ok (rnode_avail (n) == 2, "rnode_avail now is 2");
    rnode_avail_check (n, "0-1");
    ok (rnode_free (n, "0") < 0 && errno == EEXIST,
        "rnode_free of already available id fails");
    ok (rnode_avail (n) == 2, "rnode_avail is still 2");
    ok (rnode_free (n, "3") == 0, "rnode_free '3' works");
    ...
    ok (rnode_alloc_idset (n, alloc) == 0, "rnode_alloc_idset (1,3)");
    ...
    ok (rnode_free_idset (n, alloc) == 0, "rnode_free_idset (1,3)");
    rnode_avail_check (n, "0-3");
