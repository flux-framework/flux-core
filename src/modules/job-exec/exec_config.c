...
{
    return exec_conf.sdexec_constrain_resources ? true : false;
}
...
int config_get_stats (json_t **config_stats)
{
    ...
    if (!(o = json_pack ("{s:s? s:s? s:s? s:s? s:i s:f s:i s:i s:i}",
                         ...
                         "sdexec_constrain_resources",
                         exec_conf.sdexec_constrain_resources))) {
        errno = ENOMEM;
        return -1;
    }
                  ...
                  flux_error_t *errp)
{
    ...
    /*  Check configuration for exec.sdexec-constrain-resources */
    if (flux_conf_unpack (conf,
                          &err,
                          "{s?{s?b}}",
                          "exec",
                            "sdexec-constrain-resources",
                              &tmpconf.sdexec_constrain_resources) < 0) {
        errprintf (errp,
                   "error reading config value"
                   " exec.sdexec-constrain-resources: %s",
                   err.text);
        return -1;
    }
