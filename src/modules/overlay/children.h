 ...
 * there was an online→offline transition
 */
...
/* Check if children bind socket has been created.
 * Returns true if bind_zsock is non-NULL.
 */
bool children_is_bound (struct children *ctx);

/* Authorize a child connection via ZAP.
