...
{
    sqlite3_stmt *checkpt_get_stmt = NULL;
    json_t *o = NULL;
    char *s;
    int rv = -1;
    ...
    if (sqlite3_bind_text (checkpt_get_stmt,
                           1,
    ...
    if (sqlite3_step (checkpt_get_stmt) != SQLITE_ROW)
        goto drop;

    s = (char *)sqlite3_column_text (checkpt_get_stmt, 0);
...
error:
    if (checkpt_get_stmt) {
        if (sqlite3_finalize (checkpt_get_stmt) != SQLITE_OK)
            log_sqlite_error (ctx, "sqlite_finalize checkpt_get_stmt");
    }
    json_decref (o);
    return rv;
...
static int content_sqlite_checkpt_setup (struct content_sqlite *ctx)
{
    sqlite3_stmt *table_list_stmt = NULL;
    bool checkpt_found = false;
    int rv = 0;
    ...
    while (sqlite3_step (table_list_stmt) == SQLITE_ROW) {
        char *s = (char *)sqlite3_column_text (table_list_stmt, 0);
    ...
    if (checkpt_found) {
        if (content_sqlite_checkpt_migrate (ctx) < 0)
            goto cleanup;
    }
...
int mod_main (flux_t *h, int argc, char **argv)
{
    ...
    if (content_sqlite_opendb (ctx, truncate) < 0)
        goto done;
    if (content_sqlite_checkpt_setup (ctx) < 0)
        goto done;
