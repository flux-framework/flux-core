...
void basic_api_tests (void)
{
    ...
    ok (!strcmp (root->ref, "foobar"), "kvsroot_setroot set ref correctly");
    ...
    ok (kvsroot_check_user (krm, root, FLUX_ROLE_USER, 0) < 0 && errno == EPERM,
        "kvsroot_check_user fails with EPERM on role user and incorrect id");

    ok (kvsroot_check_user (krm, root, 0, 0) < 0 && errno == EPERM,
...
void basic_kvstxn_mgr_tests (void)
{
    ...
    ops = json_pack ("[{s:s s:i s:n}]", "key", "a.b.c", "flags", 0, "dirent");

    ok (kvstxn_mgr_add_transaction (root->ktm, "foo", ops, 0) == 0,
