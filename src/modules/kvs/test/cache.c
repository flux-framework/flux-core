...
void wait_error_cb (void *arg)
{
    ...
    ok (we->errnum == ENOTSUP, "wait error called correctly before callback");
    we->count++;
}
...
void cache_tests (void)
{
    ...
    ok (cache_count_entries (cache) == 0, "cache contains 0 entries");
...
void cache_entry_basic_tests (void)
{
    ...
    ok (cache_entry_create (NULL) == NULL && errno == EINVAL,
        "cache_entry_create fails with EINVAL on bad input");

    cache_entry_destroy (NULL);
    diag ("cache_entry_destroy accept NULL arg");

    ok (cache_entry_set_treeobj (NULL, NULL) < 0 && errno == EINVAL,
        "cache_entry_set_treeobj fails with EINVAL with bad input");

    ok ((e = cache_entry_create ("a-reference")) != NULL, "cache_entry_create success");

    o = json_string ("yabadabadoo");

    ok (cache_entry_set_treeobj (e, o) < 0 && errno == EINVAL,
    ...
    ok (cache_entry_set_raw (e, data, -1) < 0 && errno == EINVAL,
        "cache_entry_set_raw fails with EINVAL with bad input");
    ok (cache_entry_set_raw (e, NULL, 5) < 0 && errno == EINVAL,
        "cache_entry_set_raw fails with EINVAL with bad input");

    free (data);

    ok (cache_entry_set_errnum_on_valid (NULL, EPERM) < 0 && errno == EINVAL,
        "cache_entry_set_errnum_on_valid returns EINVAL with bad input");

    ok (cache_entry_set_errnum_on_valid (e, 0) < 0 && errno == EINVAL,
        "cache_entry_set_errnum_on_valid returns EINVAL with bad errnum");

    ok (cache_entry_set_errnum_on_notdirty (NULL, EPERM) < 0 && errno == EINVAL,
        "cache_entry_set_errnum_on_notdirty returns EINVAL with bad input");

    ok (cache_entry_set_errnum_on_notdirty (e, 0) < 0 && errno == EINVAL,
...
void cache_entry_raw_tests (void)
{
    ...
    ok ((e = cache_entry_create ("a-reference")) != NULL, "cache_entry_create works");
    ok (cache_entry_get_valid (e) == false, "cache entry initially non-valid");
    ok (cache_entry_get_dirty (e) == false, "cache entry initially not dirty");
    ...
    ok (datatmp && strcmp (datatmp, data) == 0, "raw data matches expected string");
    ok (datatmp && (len == strlen (data) + 1),
        "raw data length matches expected length");

    ok (cache_entry_set_dirty (e, true) == 0, "cache_entry_set_dirty success");
    ok (cache_entry_get_dirty (e) == true, "cache entry succcessfully set dirty");

    ok (cache_entry_clear_dirty (e) == 0, "cache_entry_clear_dirty success");
    ok (cache_entry_get_dirty (e) == false,
    ...
    ok (cache_entry_get_dirty (e) == false, "cache entry succcessfully now not dirty");
    ...
    ok ((e = cache_entry_create ("a-reference")) != NULL, "cache_entry_create works");
    ok (cache_entry_set_raw (e, NULL, 0) == 0, "cache_entry_set_raw success");
    ok (cache_entry_get_valid (e) == true,
        "cache entry now valid after cache_entry_set_raw call");
    ok (cache_entry_set_raw (e, NULL, 0) == 0,
        "cache_entry_set_raw again, silent success");
    ok (cache_entry_set_raw (e, data, strlen (data) + 1) < 0 && errno == EBADE,
        "cache_entry_set_raw fails with EBADE, changing validity type");

    o1 = treeobj_create_val ("foo", 3);
    ok (cache_entry_set_treeobj (e, o1) < 0 && errno == EBADE,
    ...
    ok (cache_entry_get_raw (e, (const void **)&datatmp, &len) == 0,
        "raw data retrieved from cache entry");
    ok (datatmp == NULL, "raw data is NULL");
    ok (len == 0, "raw data length is zero");
...
void cache_entry_raw_and_treeobj_tests (void)
{
    ...
    ok ((e = cache_entry_create ("a-reference")) != NULL, "cache_entry_create works");
    ok (cache_entry_set_treeobj (e, o1) == 0, "cache_entry_set_treeobj success");
    ok (cache_entry_get_raw (e, (const void **)&datatmp, &len) == 0,
...
void waiter_tests (void)
{
    ...
    ok (cache_entry_get_valid (e) == false, "cache entry invalid, adding waiter");
    ...
    ok (cache_entry_wait_valid (e, w) == 0, "cache_entry_wait_valid success");
    ok (cache_entry_set_raw (e, data, strlen (data) + 1) == 0,
        "cache_entry_set_raw success");
    ok (cache_entry_get_valid (e) == true, "cache entry set valid with one waiter");
    ok (count == 1, "waiter callback ran");

    we.count = 0;
    we.errnum = 0;
    ok ((w = wait_create (wait_cb, &we)) != NULL, "wait_create works");
    ok (wait_set_error_cb (w, error_cb, &we.errnum) == 0, "wait_set_error_cb works");
    ok (cache_entry_wait_valid (e, w) == 0, "cache_entry_wait_valid success");
    ok (cache_entry_set_errnum_on_valid (e, ENOTSUP) == 0,
        "cache_entry_set_errnum_on_valid success");
    ok (we.count == 1, "waiter callback ran");
    ok (we.errnum == ENOTSUP, "error callback ran");

    we.count = 0;
    we.errnum = 0;
    ok ((w = wait_create (wait_cb, &we)) != NULL, "wait_create works");
    ok (wait_set_error_cb (w, error_cb, &we.errnum) == 0, "wait_set_error_cb works");
    ok (cache_entry_wait_notdirty (e, w) == 0, "cache_entry_wait_notdirty success");
    ok (cache_entry_set_errnum_on_notdirty (e, EPERM) == 0,
        "cache_entry_set_errnum_on_notdirty success");
    ok (we.count == 1, "waiter callback ran");
    ok (we.errnum == EPERM, "error callback ran");

    count = 0;
    ok ((w = wait_create (wait_cb, &count)) != NULL, "wait_create works");
    ok (cache_entry_set_dirty (e, true) == 0, "cache_entry_set_dirty success");
    ok (cache_entry_get_dirty (e) == true, "cache entry set dirty, adding waiter");
    ok (cache_entry_wait_notdirty (e, w) == 0, "cache_entry_wait_notdirty success");
    ok (cache_entry_clear_dirty (e) == 0, "cache_entry_clear_dirty success");
    ok (cache_entry_get_dirty (e) == true, "cache entry still dirty, b/c of a waiter");
    ok (cache_entry_set_dirty (e, false) == 0, "cache_entry_set_dirty success");
    ok (cache_entry_get_dirty (e) == false,
    ...
    ok (cache_entry_get_dirty (e) == false,
        "cache entry set not dirty with one waiter");
    ok (count == 0, "waiter callback not called on force clear dirty");
...
void cache_blobref_tests (void)
{
    ...
    ok ((cache = cache_create ()) != NULL, "cache_create works");
    ok ((e = cache_entry_create ("abcd")) != NULL, "cache_entry_create works");
    ok (cache_insert (cache, e) == 0, "cache_insert works");
    ok ((ref = cache_entry_get_blobref (e)) != NULL, "cache_entry_get_blobref success");
    ok (!strcmp (ref, "abcd"), "cache_entry_get_blobref returned correct ref");
...
void cache_remove_entry_tests (void)
{
    ...
    ok ((e = cache_entry_create ("remove-ref")) != NULL, "cache_entry_create works");
    ok (cache_insert (cache, e) == 0, "cache_insert works");
    ...
    ok (cache_entry_set_treeobj (e, o) == 0, "cache_entry_set_treeobj success");
    json_decref (o);
    ok (cache_entry_get_valid (e) == true, "cache entry set valid with one waiter");
    ok (count == 1, "waiter callback ran");
    ok (cache_remove_entry (cache, "remove-ref") == 1,
...
void cache_expiration_tests (void)
{
    ...
    ok ((e1 = cache_entry_create ("xxx1")) != NULL, "cache_entry_create works");
    ok (cache_insert (cache, e1) == 0, "cache_insert works");
    ok (cache_count_entries (cache) == 1, "cache contains 1 entry after insert");
    ok (cache_lookup (cache, "yyy1", 0) == NULL, "cache_lookup of wrong hash fails");
    ok ((e2 = cache_lookup (cache, "xxx1", 42)) != NULL,
        "cache_lookup of correct hash works (last use=42)");
    ok (cache_entry_get_treeobj (e2) == NULL, "no treeobj object found");
    ok (cache_count_entries (cache) == 1, "cache contains 1 entry");
    memset (&ts, 0, sizeof (ts));
    ok (cache_get_stats (cache, &ts, &size, &incomplete, &dirty) == 0,
        "cache_get_stats works");
    ...
    o1 = treeobj_create_val ("foo", 3);
    ok ((e3 = cache_entry_create ("xxx2")) != NULL, "cache_entry_create works");
    ok (cache_entry_set_treeobj (e3, o1) == 0, "cache_entry_set_treeobj success");
    json_decref (o1);
    ok (cache_insert (cache, e3) == 0, "cache_insert works");
    ok (cache_count_entries (cache) == 2, "cache contains 2 entries after insert");
    ok (cache_lookup (cache, "yyy2", 0) == NULL, "cache_lookup of wrong hash fails");
    ...
    otest = treeobj_create_val ("foo", 3);
    /* XXX - json_equal takes const in jansson > 2.10 */
    ok (json_equal ((json_t *)otmp, otest) == 1, "expected treeobj object found");
    json_decref (otest);
    ok (cache_count_entries (cache) == 2, "cache contains 2 entries");
    ...
    ok (cache_entry_set_dirty (e4, true) == 0, "cache_entry_set_dirty success");
    ...
    ok (cache_entry_set_dirty (e4, false) == 0, "cache_entry_set_dirty success");
