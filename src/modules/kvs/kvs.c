...
static kvs_ctx_t *getctx (flux_t *h)
{
    ...
    if (!ctx) {
        ...
        if (ctx->rank == 0) {
            ctx->prep_w =
                flux_prepare_watcher_create (r, transaction_prep_cb, ctx);
            if (!ctx->prep_w) {
                saved_errno = errno;
                goto error;
            }
            ctx->check_w =
                flux_check_watcher_create (r, transaction_check_cb, ctx);
            if (!ctx->check_w) {
                saved_errno = errno;
                goto error;
            }
...
static int check_user (kvs_ctx_t *ctx,
                       struct kvsroot *root,
                       const flux_msg_t *msg)
{
    uint32_t rolemask;
...
static void content_store_completion (flux_future_t *f, void *arg)
{
    ...
    if (strcmp (blobref, cache_blobref)) {
        flux_log (ctx->h,
                  LOG_ERR,
                  "%s: inconsistent blobref returned",
                  __FUNCTION__);
        errno = EPROTO;
        goto error;
    }
...
static int kvstxn_cache_cb (kvstxn_t *kt, struct cache_entry *entry, void *data)
{
    ...
    if (content_store_request_send (cbd->ctx, blobref, storedata, storedatalen)
        < 0) {
        cbd->errnum = errno;
...
static int error_event_send (kvs_ctx_t *ctx,
                             const char *ns,
                             json_t *names,
                             int errnum)
{
...
static void kvstxn_apply (kvstxn_t *kt)
{
    ...
    if ((ret = kvstxn_process (kt, ctx->epoch, root->ref))
        == KVSTXN_PROCESS_ERROR) {
                                ...
                                bool *stall)
{
    ...
    if (!lh) {
        ...
        /* rootdir is optional */
        (void)
            flux_request_unpack (msg, NULL, "{ s:o }", "rootdir", &root_dirent);
...
static int namespace_create (kvs_ctx_t *ctx,
                             const char *ns,
                             uint32_t owner,
                             int flags)
{
...
static void start_root_remove (kvs_ctx_t *ctx, const char *ns)
{
    ...
    if ((root = kvsroot_mgr_lookup_root_safe (ctx->krm, ns))) {
        ...
        if (treq_mgr_iter_transactions (root->trm,
            ...
            < 0)
            flux_log_error (ctx->h,
                            "%s: treq_mgr_iter_transactions",
                            __FUNCTION__);
    }
}

static int namespace_remove (kvs_ctx_t *ctx, const char *ns)
{
    flux_msg_t *msg = NULL;
    int saved_errno, rc = -1;
    char *topic = NULL;
