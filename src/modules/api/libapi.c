...
static void api_reactor_stop (void *impl, int rc);
static void sync_msg_watchers  (ctx_t *c);


static const struct flux_handle_ops api_ops;

static int api_sendmsg (void *impl, zmsg_t **zmsg)
{
    ctx_t *c = impl;
...
static zmsg_t *api_recvmsg_putmsg (ctx_t *c)
{
...
static zmsg_t *api_recvmsg (void *impl, bool nonblock)
{
    ctx_t *c = impl;
    assert (c->magic == API_CTX_MAGIC);
    zmsg_t *zmsg = NULL;

    if (!(zmsg = api_recvmsg_putmsg (c)))
        zmsg = zfd_recv (c->fd, nonblock);
    return zmsg;
}

static int api_putmsg (void *impl, zmsg_t **zmsg)
{
    ctx_t *c = impl;
    assert (c->magic == API_CTX_MAGIC);
...
static int api_pushmsg (void *impl, zmsg_t **zmsg)
{
    ctx_t *c = impl;
    assert (c->magic == API_CTX_MAGIC);
    int oldcount = zlist_size (c->putmsg);
...
static void api_purge (void *impl, flux_match_t match)
{
    ctx_t *c = impl;
    assert (c->magic == API_CTX_MAGIC);
...
static int api_event_subscribe (void *impl, const char *s)
{
    ctx_t *c = impl;
    assert (c->magic == API_CTX_MAGIC);
    flux_t h = c->h;
...
static int api_reactor_start (void *impl)
{
    ctx_t *c = impl;
    assert (c->magic == API_CTX_MAGIC);
...
static int api_reactor_msg_add (void *impl, flux_msg_f cb, void *arg)
{
    ctx_t *c = impl;
    assert (c->magic == API_CTX_MAGIC);
    int rc = -1;
...
static void api_reactor_msg_remove (void *impl)
{
    ctx_t *c = impl;
    assert (c->magic == API_CTX_MAGIC);
    c->msg_cb = NULL;
...
static int api_reactor_fd_add (void *impl, int fd, int events,
...
static void api_reactor_fd_remove (void *impl, int fd, int events)
{
    ctx_t *c = impl;
    assert (c->magic == API_CTX_MAGIC);
    char hashkey[HASHKEY_LEN];
...
static int api_reactor_zs_add (void *impl, void *zs, int events,
...
static void api_reactor_zs_remove (void *impl, void *zs, int events)
{
    ctx_t *c = impl;
    assert (c->magic == API_CTX_MAGIC);
    char hashkey[HASHKEY_LEN];
...
static int api_reactor_tmout_add (void *impl, unsigned long msec, bool oneshot,
...
static void api_reactor_tmout_remove (void *impl, int timer_id)
{
    ctx_t *c = impl;
    assert (c->magic == API_CTX_MAGIC);
    char hashkey[HASHKEY_LEN];
...
static void api_fini (void *impl)
{
    ctx_t *c = impl;
    assert (c->magic == API_CTX_MAGIC);
...
flux_t flux_api_openpath (const char *path, int flags)
{
    ...
    c->h = flux_handle_create (c, &api_ops, flags);
    return c->h;
error:
    if (c)
        api_fini (c);
...
flux_t flux_api_open (void)
{
    ...
    if ((val = getenv ("FLUX_API_PATH"))) {
        ...
        snprintf (path, sizeof (path), "%s/flux-api", flux_get_tmpdir ());
    }
...
static const struct flux_handle_ops api_ops = {
    ...
    .rank = api_rank,
    .reactor_stop = api_reactor_stop,
    .reactor_start = api_reactor_start,
    .reactor_fd_add = api_reactor_fd_add,
    .reactor_fd_remove = api_reactor_fd_remove,
    .reactor_zs_add = api_reactor_zs_add,
    .reactor_zs_remove = api_reactor_zs_remove,
    .reactor_tmout_add = api_reactor_tmout_add,
    .reactor_tmout_remove = api_reactor_tmout_remove,
    .reactor_msg_add = api_reactor_msg_add,
    .reactor_msg_remove = api_reactor_msg_remove,
    .impl_destroy = api_fini,
};
