...
/* job-ingest takes in signed jobspec submitted through flux_job_add(),
 ...
 * 5) publish "job-ingest.add" event announcing new jobid
 *
 ...
 *   {"ids":[I,I,...]}
 *
 * The jobid is returned to the user in response to the "add" RPC.
 ...
 * its DOTHEX encoding.  Original add request message is copied
...
/* Create a 'struct batch', a container for a group of job add
...
static void batch_event_pub (struct batch *batch)
{
    ...
    if (!(f = flux_event_publish_pack (h, "job-ingest.add", 0, "{s:O}",
                                       "ids", batch->idlist))) {
...
/* Handle "job-ingest.add" request to add a new job.
 ...
 */
static void add_cb (flux_t *h, flux_msg_handler_t *mh,
...
static const struct flux_msg_handler_spec htab[] = {
    { FLUX_MSGTYPE_REQUEST,  "job-ingest.add", add_cb, FLUX_ROLE_USER },
