                               ...
                               struct shell_builtin *sb)
{
    ...
    if (flux_plugin_aux_set (p, "flux::shell", shell, NULL) < 0
        ...
        || flux_plugin_add_handler (p, "shell.validate", sb->validate) < 0
        || flux_plugin_add_handler (p, "shell.init",     sb->init) < 0
        || flux_plugin_add_handler (p, "shell.exit",     sb->exit) < 0
        || flux_plugin_add_handler (p, "task.init",      sb->task_init) < 0
        || flux_plugin_add_handler (p, "task.fork",      sb->task_fork) < 0
        || flux_plugin_add_handler (p, "task.exec",      sb->task_exec) < 0
        || flux_plugin_add_handler (p, "task.exit",      sb->task_exit) < 0)
        return -1;
