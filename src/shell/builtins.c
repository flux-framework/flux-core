                               ...
                               struct shell_builtin *sb)
{
    ...
    if (flux_plugin_aux_set (p, "flux::shell", shell, NULL) < 0
        ...
        || flux_plugin_add_handler (p, "flux_shell_validate",  sb->validate) < 0
        || flux_plugin_add_handler (p, "flux_shell_init",      sb->init) < 0
        || flux_plugin_add_handler (p, "flux_shell_exit",      sb->exit) < 0
        || flux_plugin_add_handler (p, "flux_shell_task_init", sb->task_init) < 0
        || flux_plugin_add_handler (p, "flux_shell_task_fork", sb->task_fork) < 0
        || flux_plugin_add_handler (p, "flux_shell_task_exec", sb->task_exec) < 0
        || flux_plugin_add_handler (p, "flux_shell_task_exit", sb->task_exit) < 0)
        return -1;
