...
test_expect_success 'job-exec: can specify default-shell on cmdline' '
	...
	flux module stats -p config.default_job_shell job-exec > stats1.out &&
...
test_expect_success 'job-exec: job-shell can be set in exec conf' '
	...
	EOF
	flux start --config-path=${name}.toml -s1 \
		flux module stats -p config.default_job_shell job-exec > ${name}.out 2>&1 &&
...
test_expect_success 'job-exec: update default shell via config reload' '
	flux module reload -f job-exec &&
	flux module stats -p config.default_job_shell job-exec > reload1A.out 2>&1 &&
	...
	EOF
	flux config reload &&
	flux module stats -p config.default_job_shell job-exec > reload1B.out 2>&1 &&
...
test_expect_success 'job-exec: cmdline default shell takes priority' '
	...
	flux module stats -p config.default_job_shell job-exec > reload2A.out 2>&1 &&
	...
	EOF
	flux config reload &&
	flux module stats -p config.default_job_shell job-exec > reload2B.out 2>&1 &&
...
test_expect_success 'job-exec: can specify imp path on cmdline' '
	...
	flux module stats -p config.flux_imp_path job-exec > stats2.out &&
...
test_expect_success 'job-exec: imp path can be set in exec conf' '
	...
	EOF
	flux start --config-path=${name}.toml -s1 \
		flux module stats -p config.flux_imp_path job-exec > ${name}.out 2>&1 &&
...
test_expect_success 'job-exec: update imp path via config reload' '
	flux module reload -f job-exec &&
	test_must_fail flux module stats -p config.flux_imp_path job-exec > reload3A.out 2>&1 &&
	cat <<-EOF > ${FLUX_CONF_DIR}/exec.toml &&
	[exec]
	imp = "/path/reload-imp"
	EOF
	flux config reload &&
	flux module stats -p config.flux_imp_path job-exec > reload3B.out 2>&1 &&
...
test_expect_success 'job-exec: cmdline imp path takes priority' '
	...
	flux module stats -p config.flux_imp_path job-exec > reload4A.out 2>&1 &&
	...
	flux dmesg -C &&
	flux config reload &&
	flux module stats -p config.flux_imp_path job-exec > reload4B.out 2>&1 &&
...
test_expect_success 'job-exec: can specify exec service on cmdline' '
	...
	flux module stats -p config.exec_service job-exec > stats3.out &&
	grep "foo" stats3.out
'
test_expect_success 'job-exec: exec service can be set in exec conf' '
	...
	EOF
	flux start --config-path=${name}.toml -s1 \
		flux module stats -p config.exec_service job-exec > ${name}.out 2>&1 &&
	grep "bar" ${name}.out
'
# N.B. exec service defaults to rexec
test_expect_success 'job-exec: update exec service via config reload' '
	flux module reload -f job-exec &&
	flux module stats -p config.exec_service job-exec > reload5A.out 2>&1 &&
	...
	EOF
	flux config reload &&
	flux module stats -p config.exec_service job-exec > reload5B.out 2>&1 &&
...
test_expect_success 'job-exec: cmdline exec service takes priority' '
	...
	flux module stats -p config.exec_service job-exec > reload6A.out 2>&1 &&
	...
	EOF
	flux config reload &&
	flux module stats -p config.exec_service job-exec > reload6B.out 2>&1 &&
...
test_expect_success 'job-exec: sdexex properties can be set in exec conf' '
	...
	EOF
	flux start --config-path=${name}.toml -s1 \
		flux module stats -p config.sdexec_properties job-exec > ${name}.out 2>&1 &&
...
test_expect_success 'job-exec: update sdexec properties via config reload' '
	flux module reload -f job-exec &&
	test_must_fail flux module stats -p config.sdexec_properties job-exec > reload8A.out 2>&1 &&
	cat <<-EOF > ${FLUX_CONF_DIR}/exec.toml &&
	[exec]
	service = "sdexec"
	[exec.sdexec-properties]
	...
	EOF
	flux config reload &&
	flux module stats -p config.sdexec_properties job-exec > reload8B.out 2>&1 &&
...
test_expect_success 'job-exec: bad sdexec properties leads to error on via config reload' '
	flux config reload &&
	flux module reload -f job-exec &&
	test_must_fail flux module stats -p config.sdexec_properties job-exec > reload9A.out 2>&1 &&
	cat <<-EOF > ${FLUX_CONF_DIR}/exec.toml &&
	[exec]
	service = "sdexec"
	[exec.sdexec-properties]
