...
test_expect_success 'flux-run: job that calls shape plugin has resources set properly' '
	...
	EOF
	flux run --r-shape=slot=4/node --dry-run hostname | jq .resources >  output1.json &&
...
test_expect_success 'flux-submit: job with invalid shape is rejected' '
	test_must_fail flux alloc --r-shape=slot/ echo hello 2> err.out &&
...
test_expect_success 'flux-batch: subinstance job with shape plugin has proper resources' '
	...
	EOF
	flux batch --r-shape=node/[slot=10{read-db}/[core\;memory=4{unit:GB}]\;slot{db}/[core=6\;memory=24{unit:GB}]] \
...
'
test_expect_success 'flux-submit: providing --r-json gets the expected resource set' '
	cat <<-EOF > r_set.json &&
	{
	  "resources": [
	    {
	...
	EOF
	flux submit --r-json=r_set.json --dry-run hostname | jq .resources > output3.json && 
...
test_expect_success 'flux-submit: providing invalid json gets rejected' '
	...
	EOF
	test_must_fail flux submit --r-json=r_set2.json hostname 2> output4.err &&
