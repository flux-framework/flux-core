...
test_expect_success 'flux-alloc: job that calls a plugin has jobspec set properly' '
	...
	test "$(jq -r .attributes.system.fluxion_match_policy jobspec1.json)" \
		= "firstnodex"
'
...
test_expect_success 'flux-alloc: job with invalid plugin key is rejected outright' '
	test_must_fail flux alloc -N1 --ex-junk --ex-notthis=one -N 1 \
		echo hello 2> out2.err &&
...
test_expect_success 'flux-alloc: first plugin in FLUX_CLI_PLUGINPATH is honored in name collisions' '
	export FLUX_CLI_PLUGINPATH=${SHARNESS_TEST_SRCDIR}/cli-plugins/extras/:$FLUX_CLI_PLUGINPATH &&
	flux run --help 2> out6.err 1> out6.out &&
	grep "I am the first plugin and I got loaded" out6.out
'
