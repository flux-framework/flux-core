...
'
test_expect_success 'flux-jobs config may omit description' '
	cat <<-EOF >dir/flux/flux-jobs.toml &&
	[formats.myformat]
	format = "{id.words}"
	EOF
...
'
test_expect_success 'flux-jobs does not allow internal formats to be overridden' '
	cat <<-EOF >dir/flux/flux-jobs.toml &&
	[formats.default]
	description  = "override of default format"
	format = "{id}"
	EOF
	( export XDG_CONFIG_HOME="$(pwd)/dir" &&
	  test_must_fail flux jobs --format=help >no-override.out 2>&1
	) &&
	grep "override of builtin format" no-override.out
...
'
test_expect_success 'Invalid flux-jobs invalid config causes meaningful error' '
	mkdir -p dir/flux &&
	cat <<-EOF >dir/flux/flux-jobs.toml &&
	[formats.myformat]
