...
test_expect_success 'duration and expiration expected values' '
	flux job list -s inactive | grep $(cat runupdate.id) | jq -e ".duration == 600.0" &&
	current=$(date +%s) &&
...
test_expect_success 'duration and expiration updated as expected' '
	...
	flux job list -s active | grep $(cat runupdate.id) | jq -e ".expiration > $(cat test_expiration3.out)"
'
