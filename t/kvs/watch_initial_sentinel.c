...
static const struct option longopts[] = {
   ...
   {"lookup_get_treeobj", no_argument, 0, 't'},
   {0, 0, 0, 0},
};
...
{
    fprintf (stderr,
             "Usage: watch_initial_sentinel [-w] [-W] [-rt] <key>\n");
    exit (1);
}
...
void lookup_get_treeobj (flux_future_t *f)
{
    const char *treeobj;
    if (flux_kvs_lookup_get_treeobj (f, &treeobj) < 0) {
        if (errno != ENODATA)
            log_err_exit ("flux_kvs_lookup_get_treeobj");
        flux_future_destroy (f);
        return;
    }
    if (!treeobj)
...
void lookup_continuation (flux_future_t *f, void *arg)
{
    ...
    case LOOKUP_GET_RAW:
        lookup_get_raw (f);
        break;
    case LOOKUP_GET_TREEOBJ:
        lookup_get_treeobj (f);
        break;
    default:
