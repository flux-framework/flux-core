...
# N.B. treeobj's have data in base64, "MQ==" is "1" in base64 and
# "NDI==" is "42" in base64
test_expect_success NO_CHAIN_LINT 'watch-initial-sentinel basic works (get_treeobj)' '
	flux kvs put test.sentinel.test4=1
	${FLUX_BUILD_DIR}/t/kvs/watch_initial_sentinel -t \
		test.sentinel.test4 > sentinel4.out &
	pid=$! &&
	wait_watcherscount_nonzero primary &&
	flux kvs put --append test.sentinel.test4=42 &&
	$waitfile --count=1 --timeout=10 \
		  --pattern="NDI" sentinel4.out >/dev/null &&
	kill -s USR1 $pid &&
        wait $pid &&
	test_debug "cat sentinel4.out" &&
	head -n 1 sentinel4.out | grep "MQ" &&
        tail -n 2 sentinel4.out | head -n 1 | grep sentinel &&
	tail -n 1 sentinel4.out | grep "NDI"
'
