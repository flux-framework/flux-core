...
test_expect_success 'kvs: treeobj of all types handled by get --treeobj' '
	...
	flux kvs link --link-namespace=A bar $DIR.symlinkNS &&
...
test_expect_success 'kvs: symlink w/ Namespace works' '
	...
	flux kvs link --link-namespace=TESTSYMLINKNS $TARGET $DIR.symlinkNS &&
...
test_expect_success 'kvs: symlink w/ Namespace fails on bad namespace' '
	TARGET=$DIR.target &&
	flux kvs put --json $TARGET=\"foo\" &&
	flux kvs link --link-namespace=TESTSYMLINKNS-FAKE $TARGET $DIR.symlinkNS &&
...
test_expect_success 'kvs: readlink works with nslnks (multiple inputs)' '
	...
	flux kvs link --link-namespace=TESTSYMLINKNS $TARGET1 $DIR.symlinkNS1 &&
	flux kvs link --link-namespace=TESTSYMLINKNS $TARGET2 $DIR.symlinkNS2 &&
...
test_expect_success 'kvs: symlinkNS: path resolution when intermediate component is a link' '
	...
	flux kvs link --link-namespace=TESTSYMLINKNS $DIR.a.b $DIR.Z.Y &&
	OUTPUT=$(flux kvs get --json $DIR.Z.Y.c) &&
	test "$OUTPUT" = "42"
'
test_expect_success 'kvs: symlinkNS: intermediate link points to another namespace link' '
	...
	flux kvs --namespace=TESTSYMLINKNS unlink -Rf $DIR &&
	flux kvs --namespace=TESTSYMLINKNS put --json $DIR.a.b.c=42 &&
	flux kvs --namespace=TESTSYMLINKNS link --link-namespace=TESTSYMLINKNS $DIR.a.b $DIR.Z.Y &&
	flux kvs link --link-namespace=TESTSYMLINKNS $DIR.Z.Y $DIR.X.W &&
	test_kvs_key $DIR.X.W.c 42
'
...
test_expect_success 'kvs: symlinkNS: put cant cross namespace' '
	...
	flux kvs link --link-namespace=TESTSYMLINKNS $DIR.a $DIR.link &&
	! flux kvs put --json $DIR.link.X=42
'
test_expect_success 'kvs: symlinkNS: dangling link' '
	flux kvs unlink -Rf $DIR &&
	flux kvs --namespace=TESTSYMLINKNS unlink -Rf $DIR &&
	flux kvs link --link-namespace=TESTSYMLINKNS-FAKE $DIR.dangle $DIR.a.b.c
'
