...
'
test_expect_success 'rexec: waitable non-background proc does not become zombie' '
	flux exec -r 0 --waitable sleep 0 &&
	$rps > waitable-nonbg-ps.out &&
	test_debug "cat waitable-nonbg-ps.out" &&
	test_must_fail grep sleep waitable-nonbg-ps.out
'
...
'
test_expect_success 'rexec: streaming RPC clears waitable flag (non-bg)' '
	flux exec -r 0 --waitable --label=stream-wait sleep 0 &&
