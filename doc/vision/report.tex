...
some of the key challenges that the new RM paradigm presents to the design space. 
They represent the main factors that \ngrm's new concepts and software design
must effective address in delivering the new capabilities described in 
...
\item{\sl Power challenge:} As a one specific example of emerging resource types,
      power as a resource is becoming increaingly important. When the computing
...
\item{\sl Integration risk:} In the new paradigm, the RM must 
     integrate other software that are essential computing elements. 
...
\item{\sl Downtime cost:} In the new paradigm, the RM must 
...
\item{Common scalable infrastructure model} Including per-job Comms session. This is the model that addresses scale challenge in concurrency along with other new capabilities noise control. {\sl Integration challenge} 

\item{Customizable job instantiation model} Describe a job can custimize itself with its own plug-ins. {\sl Integration challenge} along with other capabilities. 

\item{WRAP model} 
\end{itemize}


\section{Next Generation Resource Management}
