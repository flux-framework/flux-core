...
a wide range of compute resources in a tree-based hierarchy with
...
we have a single job ({\tt job 0.x}) managing the entire cluster resources, 
...
\item{An ``isolated'' communications framework with {\em gateway} functionality
      for relaying messages to the parent---i.e., our {\em common scalable communication infrastructure model};}
...
this case, a fully functional child job is not needed. For
efficient reource use, we introduce the
concept of a {\em lightweight job} (LWJ) for this case. LWJs are
...
of two LWJs that divides the resources of a single node.
...
By first approximation, the traditional job maps to an \ngrm\ job---i.e.,  without 
the unifed RM instance and hierarchy. The job step maps to the LWJ. 
...
to a database.  In \ngrm, the LWJ's statistics will be reaped by the
...
\ngrm, job reservations will take the form of a parent job (with a
future start time) under which user jobs will run.
Table~\ref{tab:tradterms} summaries these comparisons. 
