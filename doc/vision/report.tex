...
to choose from several equivalent resource types. This then makes the
...
model within our job hierarchy model above: a child job sends a grow/shrink 
request to its parent, which can go up the job hierarchy
...
The elasticity model addresses not only the {\sl dynamic workload} and {power challenges}
...
for these design challenges, but that approach has proven to be ineffective for HPC due in large
...
and private file system namespaces for jobs, achieving
a higher degree of isolation between system software and user-visible
