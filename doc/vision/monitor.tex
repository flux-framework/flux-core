...
\begin{figure}
\centering
\fbox{\includegraphics[scale=0.1]{mon_iface}}
...
\fbox{\includegraphics[scale=0.1]{mon_ex1a}}
\end{minipage}
\hspace{1cm}
\begin{minipage}[b]{0.4\linewidth}
\fbox{\includegraphics[scale=0.1]{mon_ex1b}}
\end{minipage}
...
MyLC and the new HM will both be providing
notifications for various events, so we need to develop a centralized
mechanism that can support this and make sure \ngrm\ can be plugged-in as
an event source. (Sounds like the pub-sub model Mark described would work
nicely.
{\bf jg:}
FIXME: This section describes how \ngrm\ can act as an SNMP event source
...
implementations.  The gateway is just a thin layer between resource db
and external protocol engine.}
\fi
