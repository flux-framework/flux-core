...
within this bound. Thus, it is WRAP's responsibility to ensure that
these transactions get executed most efficiently within this scheduler-set bound.
...
management, WRAP must provide advanced run-time services to 
implement conceptual models such as job hierarchy and resource elasticity 
models described in Section~\ref{sect:models}.
...
paradigm models transactions that a job executes simply as a set of
compute steps---e.g., {\tt job steps}.
As with other limitations, this simple traditional model
is ill-suited for designing our run-time services after. 
Rather, WRAP requires a more powerful and flexible mechanism
to organize and group the processes that a job executes 
in accordance with their distinct functions. For example, all of the
parallel processes of an MPI application may form a single 
compute function; all of the distributed processes of a parallel
debugger program may form a separate tool function, acting on 
the compute function; further, the processes in the compute function
may refine itself into several sub-groups, and each serves
an independent power capping function to its member processes.  

We use the notion of the lightweight
job (LWJ) to realize this model. An LWJ is a group of processes with a distinct function 
that has its own resource confinement, a subset of resources assigned to the job.
A signficant difference with the full job is that an LWJ
can share the resources with other LWJs of the same job. 
Further, because the processes grouped by an LWJ can be refined into smaller LWJs,
LWJs also form a hierarchy, bringing  
our general {\em job hierachy model} into the 
within-job scope and following the same ``parent-child'' rules stated
...
locate, synchronize with, and attach to MPI processes grouped through an compute LWJ.
Similarly, WRAP may move a portion of the resources (e.g., maximum power use) that one LWJ has been using to 
another LWJ. That forms the basis for our elasticity model at the within-job scope.


%Using the concept of LWJs, the main goal of WRAP design can 
%be stated as providing effective and scalable run-time services 
%to map a {\em LWJ hierarchy} to a resource allocation
%that also forms a hierarchy as shown in Figure~\ref{fig:jobrm}.
...
For example, $j$ represents an LWJ in the hierarchy within its job and 
$r_j$ denotes the compute resources to that this LWJ is confined.
Using those as the foundational parameters, WRAP defines
...
\item{$split(j, marker)$: creates a new LWJ that includes all of the calling processes of $j$ with the same marker.}
...
by composing primitives in Section~\ref{sect:prim}.
...
\subsubsection{Advanced Power-Aware Run-time Support}
The hierarchy of LWJs allows varying grain of control for both the size of
an LWJ in terms of its process count and resource distribution
across the LWJs.
This has many interesting properties. For example, an MPI
LWJ can be further refined into smaller LWJs
representing groups of MPI processes. 
As smaller LWJs allocate, grow or shrink within the resource
bound set by their parent MPI LWJ, consumable resources
like power can be unevenly or evenly distributed across
those smaller LWJs.
This mechanism can support emerging power-aware computing research
that may want to cap power use differently across groups
of processes based on their execution patterns.
