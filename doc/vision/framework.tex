...
security context or, or using MUNGE\cite{MUNGE}.
...
Several software subsystems in \ngrm\ will be extensible via plugins.
Although each subsystem will likely have unique API's for plugins
that match their specific funcionality, a common plugin architecure
should be implemented to make extending \ngrm\ consistent across
subsystems.  The plugin architecture should provide isolation between
plugin code and the internals of the subsystem.  For example,
ideally it should not be possible to cause the main code to segfault or
deadlock by loading a malfunctioning plugin.
...
\begin{longtable}{|p{1cm}|p{10.2cm}|p{1cm}|p{1cm}|p{1.8cm}|}\hline
  ...
  \hline
  1.1.1.  & 0MQ evaluation.
          Consider CMB and aggregation/reduction design based on 0MQ.
  ...
  1.1.11. & Implement aggregation/reduction framework.
	...
	& 1.1.10\\
  \hline
  \multicolumn{5}{|l|}{1.2. \textbf{Plugin Architecture}} \\
  \hline
  1.2.1 & Survey plugin architectures such as SPANK, PAM, eclipse,
	  and chrome.  Identify essential elements that make them
          succesful.
	& V
	& 
	& \\
  \hline
  1.2.2 & Design/prototype common plugin architecture with examples.
          Document project plugin guidelines.
	& DR, D
	& 
	& 1.2.1\\
  \hline
