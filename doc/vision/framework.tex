...
the IP transport, comms toolkit, and comms message bus, shown
...
Within a comms session, a distributed comms message bus (CMB)
...
detecting and adapting to session node failures, and
providing a basic event messaging system.

\paragraph{Architecture}
Although we refer to it as a message bus because its function
is analagous to that of system-dbus on Linux, the CMB is a distributed
service with nodes interconnected in a topology that is to be determined.
...
The CMB implements a simple key-value store to manage a small amount of
internal state as shown in Table~\ref{TabCMBState}.
The session state is small enough, say $< 0.5\%$ RAM,
to be fully replicated on every node in the session.
The master state lives on the control node.
Slave copies on other nodes are loosely consistent with the master copy:
reads are local, writes are through to the control node.
Each write on the control node increments a generation number and
triggers a state update event.
Nodes joining (or rejoining) the session pull down a fresh copy of the
state, then process update events to stay synchronized.
...
\begin{table}
  ...
  cmb.topology.up.$node$ = $nodelist$ & 
        Upward peers for CMB topology, TBD.\\
  cmb.topology.dn.$node$ = $nodelist$ &
        Downward peers for CMB topology, TBD.\\
  cmb.alive.$node$ = $down|up$ &
        Liveness for each node.\\
  cmb.alloc.$node$ = $session|nil$ &
        Child session node is allocated to or nil if not allocated.\\
  cmb.child.$session$.cred = $key$ &
        Child sesion key.\\
  cmb.child.$session$.control = $nodelist$ &
        Control node(s) for child session.\\
  \hline
  \end{tabular}
  \caption{Comms Message Bus State}
  \label{TabCMBState}
\end{table}
...
child session key) and brings it back online, de-allocated, in the parent
sesssion.  
...
of trigger periods, it is marked {\em down}.
...
child comms sessions as shown in Figure~\ref{FigCMBSpawn}.
