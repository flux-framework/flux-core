...
If _ns_ is NULL, `flux_kvs_commit()` will operate on the namespace
returned via `flux_kvs_get_namespace()`.  Note that all transactions
operate on the same namespace.
