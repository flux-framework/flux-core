...
and a call to `flux_rpc_get(3)` or `flux_rpc_getf(3)` would not block.
...
RPC response is received.  It should call `flux_rpc_get(3)` or
`flux_rpc_getf(3)` to obtain the RPC result.
