...
execs a single flux-broker(1) directly.  If a PMI environment is
available, flux-broker will use it to fetch job information and
bootstrap a session of geometry determined from PMI.  If PMI is
unavailable, flux-broker will run as a standalone, single-rank job.
