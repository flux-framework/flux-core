...
an RFC 11 snapshot reference.  If the link also points to a namespace,
the namespace and key will be output in the format
'<namespace>::<key>'.  The '-o' can be used to only output namespaces
and the '-k' can be used to only output keys.
