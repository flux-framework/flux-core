...
Multi-part ZeroMQ messages are displayed separated by a horizontal
...
the ZeroMQ DEALDER - ROUTER flow, socket identites of each hop in
the request's route are prepended as it is routed "upstream" towards
...
with abbrevieated addresses separated by exclaimation points, and the number
of hops shown in brackets, not the size.  Flux responses utilize the
reverse flow, where routing frames are stripped of and used to select
the next peer, but otherwise have the same structure as requests.

Events consist of a string message part (the topic name) followed by
...
*-l ,--long*::
Display long fromat of message dumps.  Do not truncate at 80 colums.
