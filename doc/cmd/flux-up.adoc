...
ensure a message rate no less than the session heartrate.  If idle time for
...
tree-based overlay network, that they should choose a new parent
(normally a grandparent).

The algorithm for changing node state and updating the topology is
implemented in the 'live' comms module.
