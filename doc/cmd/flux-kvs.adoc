...
service used a building block by other framework components.
flux-kvs(1) is a command line utility that operates on the KVS.
It is a thin layer on top of the C API for the Flux KVS.

The Flux KVS stores JSON values under string keys.  The keys are
hierarchical, using "." as a path separator analagous to "/" separated
...
of synchronization is:  node A puts a value, reads version, send version
to node B.  Node B waits for version, gets value.
