...
"flux -h" summarizes the available flux-core commands.

If 'CMD' contains a slash "/" character, it is assumed to be
a sub-command in development and is executed directly, bypassing
the sub-command search path.

"flux -h CMD" is equivalent to "flux CMD -h".
...
*-x, --exec-path*='PATH'::
Prepend 'PATH' to subcommand executable search path.
...
-----------------------
flux(1) uses compiled-in install paths, a config file, the environment,
and its command line arguments to up the environment for sub-commands.

Sub-command search path::
Look for "flux-'CMD'" executable by searching a path constructed
with the following prototype:

  [--exec-path PATH]:[config-file exec_path]:install-path

setenv FLUX_MODULE_PATH::
Set up broker comms module search path according to:

  [--module-path PATH]:[config-file module_path]:install-path

setenv LUA_PATH::
Set Lua module search path:

  [--lua-path PATH];[config-file lua_path];[getenv LUA_PATH]; \
    install-path;;;

setenv LUA_CPATH::
Set Lua binary module search path:

  [--lua-cpath PATH];[config-file lua_cpath];[getenv LUA_CPATH]; \
    install-path;;;

setenv FLUX_CMBD_PATH::
Set the path to the broker executable.  This environment variable is
used by flux-start(1) and related subcommands to launch the broker.
The path may be set by any of the following methods, in decreasing precedence:
- [--cmbd-path FILE]
- [config-file cmbd_path]
- install-path

setenv FLUX_CONFIG::
Set the path to the config file.  The config file location is used by flux(1),
and the environment variable may be used by subcommands.
The path may be set by any of the following methods, in descending precedence:
- [--config FILE]
- [top of build tree, if being run from flux-core repo]
- [$HOME/.flux/config]
...
Flux subcommand from "outside" of a comms session, e.g. not as a process
descendent of the broker, you may need to set this to find the correct
API socket.
