...
. The front-end command ('wreckrun') allocates a new job directory
  entry in the kvs under the 'lwj' key. This operation is performed
  as a 'job.create' request, which results in a new directory in
  the kvs as 'lwj.<jobid>'. Initially, a job state is stored in
  'lwj.<jobid>.state' as 'reserved'.
. The front-end command now fills in requisite and optional job
  information under the 'lwj.<jobid>' hierarchy such as the
  command-line to run, how many tasks to run on each rank, the
  job environment, etc. 
...
------------------------------
$ flux module list | egrep 'resrc|job|wrexec'
wrexec               686108          83 [0-3]
resrc                667324          83 [0-3]
job                  666696          83 0
$
------------------------------
