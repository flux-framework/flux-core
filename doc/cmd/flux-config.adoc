...
DESCRIPTION
-----------
A Flux configuration file is used by the flux(1) command driver
to determine various search paths.  This file currently has limited
usage in the Flux framework source trees, as a mechanism for configuring
in-tree unit test execution.  It should be considered EXPERIMENTAL
and its content subject to change.
...
For example "flux-config get /general/exec_path" would print "xyz":
