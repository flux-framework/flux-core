...
SYNOPSIS
--------
*flux-keygen* ['--force'] ['--plain'|'--curve']


DESCRIPTION
-----------
flux-keygen(1) generates long-term keys for Flux security.
Keys are written to a '.flux' subdirectory in your home directory.
...
//ZeroMQ IPC (UNIX domain socket) traffic is also protected with CURVE.
Multicast traffic is protected by enclosing messages in a MUNGE
payload with MUNGE_OPT_UID_RESTRICTION set.  
...
PLAIN keys are merely RFC 4122 uuid's stored in the clear that are
used as passwords.
...
*-p, --plain*::
Generate only PLAIN keys.

*-c,--curve*::
Generate only CURVE keys.
